...
* 依照[使用 Gateway 连接多个集群](/zh/docs/setup/kubernetes/install/multicluster/gateways/)中提到的步骤设置一个多集群环境。
...
## 在 `cluster1` 中部署 `bookinfo` 的 `v1` 版本
...
## 在 `cluster2` 中部署 `bookinfo` 的 `v2` 和 `v3`
...
根据[配置指南](/zh/docs/setup/kubernetes/install/multicluster/gateways/#配置-dns)中的介绍，远程服务可以用一个 `.global` 的 DNS 名称进行访问。在我们的案例中，就是 `reviews.default.global`，所以我们需要为这个主机创建 `ServiceEntry` 和 `DestinationRule`。`ServiceEntry` 会使用 `cluster2` 网关作为端点地址来访问服务。可以使用网关的 DNS 名称或者公共 IP：
...
本文中，我们看到在[多控制平面拓扑](/zh/docs/concepts/multicluster-deployments/#多控制平面拓扑)的多集群网格中，如何使用 Istio 路由规则进行跨集群的流量分配。
