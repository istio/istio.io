...
要配置客户端，您需要设置[目标规则](/zh/docs/concepts/traffic-management/#目标规则)以使用双向 TLS。它可以使用
...
目标规则的命名空间。
* 使用 `ISTIO_MUTUAL` TLS 模式，Istio 将根据以下内容设置密钥和证书（例如客户端证书，私钥和 CA 证书）的路径。
它的内部实施。
...
> 由于 Envoy 拒绝明文请求(即 HTTP)请求的方式，在这种情况下，您将看到 `curl` 退出代码 56（接收网络数据失败）。
...
Kubernetes API 服务器没有 sidecar，因此来自 `sleep.foo` 等 Istio 服务的请求会因发送时出现同样的问题而失败
要求任何非 Istio 服务。
...
下面的示例显示了为命名空间 `foo` 中的所有服务启用双向 TLS 的策略。正如你所看到的，它使用了 kind：“Policy”而不是“MeshPolicy”，
