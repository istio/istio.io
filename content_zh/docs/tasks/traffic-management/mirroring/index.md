...
*   首先部署启用了访问日志的两个版本的 [httpbin](/zh{{< github_tree >}}/samples/httpbin) 服务：
...
1.  创建一个默认路由规则，将所有流量路由到服务的 `v1` ：
    ...
    > 注意：如果您已经安装/配置 Istio 并启用 TLS 双向认证，您必须增加 [TLSSettings.TLSmode](/zh /docs/reference/config/istio.networking.v1alpha3/#TLSSettings-TLSmode), `mode: ISTIO_MUTUAL` 。如 [TLSSettings](/zh/docs/reference/config/istio.networking.v1alpha3/#TLSSettings) 参考中所述。
...
1. 如果您不打算探索任何后续任务，请参阅
  [Bookinfo 清理](/zh/docs/examples/bookinfo/#cleanup) 的说明去关闭应用程序。