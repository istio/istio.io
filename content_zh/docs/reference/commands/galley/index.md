...
|`--kubeConfig <string>`|Kube config 文件的路径。（缺省缺省 `''`）|
...
运行一个 htts 服务器，用来进行 Istio 的配置验证。用 Kubernetes 验证 Webhook 进行 Pilot 和 Mixer 配置的验证。
...
|`--webhook-config-file <string>`|包含 Kubernetes validatingwebhookconfiguration Yaml 的文件名。如果没有指定这一文件，验证功能关闭。（缺省值 `''`）|
