...
- [Service entry](/docs/reference/config/istio.networking.v1alpha3/#serviceentry) 校验，当配置了 DNS 解析的时候将拒绝不使用通配符主机名（`*`）。相关 API 从未允许这种行为，但在前一版本中，`ServiceEntry` 对象的验证过程错误的忽略了这一错误。
