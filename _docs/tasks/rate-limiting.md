...
If you now refresh the `productpage` while the load 
...
1. Configure mixer with the conditional rate limit:
   ...
   ```bash
   istioctl mixer rule create global ratings.default.svc.cluster.local -f samples/apps/bookinfo/mixer-rule-ratings-ratelimit-conditional.yaml
   ```
...
As in the previous example, if you refresh the `productpage` while the load 
generator is running (i.e., generating more than 1 req/s), the traffic generated by
