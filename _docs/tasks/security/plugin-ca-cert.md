...
overview: This task shows how operators can plug existing root certificate, signing certificate and key into Istio Citadel.
...
This task shows how operators can plug existing root certificate, signing certificate and key into Istio Citadel.

By default, Istio Citadel generates self-signed root certificate and key, and uses them to sign the workload certificates.
Istio Citadel can also use the operator-specified certificate and key to sign workload certificates, with
operator-specified root certificate. This task demonstrates an example to plug certificates and key into Istio Citadel.
...
Suppose we want to have Istio Citadel use the existing singing (CA) certificate `ca-cert.pem` and key `ca-key.pem`.
...
Istio Citadel's signing (CA) certificate (`ca-cert.pem`) is different from root certificate (`root-cert.pem`),
...
In our example, it contains Istio Citadel's signing certificate, so `cert-chain.pem` is the same as `ca-cert.pem`.
...
The following steps enable plugging in the certificates and key into Istio Citadel:
...
1. Redeploy Istio Citadel, which reads the certificates and key from the secret-mount files:
   ```bash
...
1. To make sure the workloads obtain the new certificates promptly,
   delete the secrets generated by Istio Citadel (named as istio.\*).
   In this example, `istio.default`. Istio Citadel will issue new certificates for the workloads.
   ```bash
