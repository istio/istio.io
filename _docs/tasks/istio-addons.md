...
1. Execute a Prometheus query.
    ...
    In the "Expression" input box at the top of the web page, enter the text: `request_count`. Then, click the **Execute** button.
...
the exposed metrics. It provides a mechanism for persistent storage and querying of Istio metrics. The Grafana add-on, for instance, uses this add-on as the source for all of the dashboard data.
...
1. *mixer* (`istio-mixer.istio-system:9093`): all Mixer-specific metrics. Used to monitor Mixer itself.
1. *envoy* (`istio-mixer.istio-system:9102`): raw stats generated by Envoy (and translated from statsd to prometheus).
...
1. To view Istio metrics in a graphical dashboard install the
   Grafana add-on. 
...
1. Send traffic to the mesh.
  ...
  <figure><img style="max-width:100%" src="./img/dashboard-with-traffic.png" alt="Istio Dashboard With Traffic" title="Istio Dashboard With Traffic" />
