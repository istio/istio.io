...
1. Determine the gateway ingress URL:
   ...
   ```
   If you are using IBM Bluemix Container service free clusters, the external load balancer is not supposed. You may obtain the public ip   for your worker VM via the output of ```bx cs workers {cluster-name}```, where ```cluster-name``` is the name or id of your cluster. 
