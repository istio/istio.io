...
with the [Helm Chart]({{home}}/docs/setup/kubernetes/helm-install.html):
   ...
   b) Install Istio and enable [mutual TLS authentication]({{home}}/docs/concepts/security/mutual-tls.html) between sidecars. This option is mostly for new clusters, i.e., all applications have a sidecar injected during their deployment. For existing applications, please choose the above option and enable mTLS using [authentication policy]({home}}/docs/tasks/security/authn-policy.html):
