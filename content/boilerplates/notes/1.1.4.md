...
- Changed the default behavior for Pilot to enable fall through routes which allows workloads to access external host using the same port as any
internal HTTP service. This behavior can be controlled by the `PILOT_ENABLE_FALLTHROUGH_ROUTE` environment variable.
...
- Fixed a bug in Pilot where it didn't remove endpoints from Envoy after they were removed from Kubernetes ([Issue 13402](https://github.com/istio/istio/issues/13402)).
