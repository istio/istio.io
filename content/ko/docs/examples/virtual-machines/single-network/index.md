...
- You have already set up Istio on Kubernetes. If you haven't done so, you can
  find out how in the [Installation guide](/ko/docs/setup/getting-started/).
...
1. Create a Kubernetes secret for your generated CA certificates using a command similar to the following. See [Certificate Authority (CA) certificates](/ko/docs/tasks/security/cert-management/plugin-ca-cert/) for more details.
...
1. For a simple setup, deploy Istio control plane into the cluster
    ...
    [installation instructions](/ko/docs/setup/install/istioctl/).

   Alternatively, the user can create an explicit service of type `LoadBalancer` and use
    [internal load balancer](https://kubernetes.io/ko/docs/concepts/services-networking/service/#internal-load-balancer)
...
1. Determine and store the IP address of the Istiod since the VMs
   access [Istiod](/ko/docs/ops/deployment/architecture/#pilot) through this IP address.
...
`/etc/hosts/`, in this case using a service from the [Bookinfo example](/ko/docs/examples/bookinfo/).
...
1. Add VM services to the mesh
    ...
    {{< tip >}}
    Ensure you have added the `istioctl` client to your path, as described in the [download page](/ko/docs/setup/getting-started/#download).
    {{< /tip >}}
