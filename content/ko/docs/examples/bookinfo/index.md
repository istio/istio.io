---
title: 첵 정보 애플리케이션
description: 다양한 이스티오 기능을 시연하는 데 사용되는 네 가지 개별 마이크로서비스로 구성된 샘플 애플리케이션을 배포한다.
weight: 10
...
{{< tip >}}
If you installed Istio using the [Getting Started](/ko/docs/setup/getting-started/)
...
in the [installation guide](/ko/docs/setup/).
...
1.  The default Istio installation uses [automatic sidecar injection](/ko/docs/setup/additional-setup/sidecar-injection/#automatic-sidecar-injection).
    ...
    {{< warning >}}
    If you use OpenShift, make sure to give appropriate permissions to service accounts on the namespace as described in [OpenShift setup page](/ko/docs/setup/platform-setup/openshift/#privileged-security-context-constraints-for-application-sidecars).
    {{< /warning >}}
...
1.  Deploy your application using the `kubectl` command:
    ...
    (/ko/docs/setup/additional-setup/sidecar-injection/#manual-sidecar-injection),
    use the [`istioctl kube-inject`](/ko/docs/reference/commands/istioctl/#istioctl-kube-inject) command to modify the `bookinfo.yaml`
...
Kubernetes cluster, e.g., from a browser. An [Istio Gateway](/ko/docs/concepts/traffic-management/#gateways)
...
1.  Follow [these instructions](/ko/docs/tasks/traffic-management/ingress/ingress-control/#determining-the-ingress-ip-and-ports) to set the `INGRESS_HOST` and `INGRESS_PORT` variables for accessing the gateway. Return here, when they are set.
...
versions, called *subsets*, in [destination rules](/ko/docs/concepts/traffic-management/#destination-rules).
...
{{< tip >}}
The `default` and `demo` [configuration profiles](/ko/docs/setup/additional-setup/config-profiles/) have [auto mutual TLS](/ko/docs/tasks/security/authentication/authn-policy/#auto-mutual-tls) enabled by default.
...
To proceed, refer to one or more of the [Istio Tasks](/ko/docs/tasks),
depending on your interest. [Configuring Request Routing](/ko/docs/tasks/traffic-management/request-routing/)
