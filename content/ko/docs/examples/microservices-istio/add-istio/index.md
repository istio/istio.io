...
1.  Redeploy the `productpage` microservice, Istio-enabled:
    ...
    however [Automatic sidecar injection](/ko/docs/ops/configuration/mesh/injection-concepts/) is more convenient.
    {{< /tip >}}
...
1.  Kubernetes replaced the original pods of `productpage` with the
    ...
    [rolling update](https://kubernetes.io/ko/docs/tutorials/kubernetes-basics/update-intro/).
...
You are ready to
[enable Istio on all the microservices](/ko/docs/examples/microservices-istio/enable-istio-all-microservices).
