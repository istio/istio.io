---
title: 이스티오 서비스 상태 점검
description: 이스티오 서비스에 대한 상태 점검 방법을 보여준다.
weight: 50
...
[Kubernetes liveness and readiness probes](https://kubernetes.io/ko/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/)
...
so that the probe request is sent to the [sidecar agent](/ko/docs/reference/commands/pilot-agent/).
...
This feature is enabled by default in all built-in Istio [configuration profiles](/ko/docs/setup/additional-setup/config-profiles/)
...
You can [annotate the pod](/ko/docs/reference/config/annotations/) with `sidecar.istio.io/rewriteAppHTTPProbers: "false"`
to disable the probe rewrite option. Make sure you add the annotation to the
[pod resource](https://kubernetes.io/ko/docs/concepts/workloads/pods/pod-overview/) because it will be ignored
...
[Install Istio](/ko/docs/setup/install/istioctl/) using `--set values.sidecarInjectorWebhook.rewriteAppHTTPProbe=false`
