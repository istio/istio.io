...
aliases:
  ...
  - /ko/docs/ops/telemetry/monitoring-multicluster-prometheus
...
[hierarchical federation](https://prometheus.io/ko/docs/prometheus/latest/federation/#hierarchical-federation).
...
Follow the [multicluster installation](/ko/docs/setup/install/multicluster/) section to setup your Istio clusters in one of the
supported [multicluster deployment models](/ko/docs/ops/deployment/deployment-models/#multiple-clusters). For the purpose of
...
Istio provides a way to expose cluster services externally via [Gateways](/ko/docs/reference/config/networking/gateway/).
You can configure an ingress gateway for the cluster-local Prometheus, providing external connectivity to the in-cluster
Prometheus endpoint.

For each cluster, follow the appropriate instructions from the [Remotely Accessing Telemetry Addons](/ko/docs/tasks/observability/gateways/#option-1-secure-access-https) task.
