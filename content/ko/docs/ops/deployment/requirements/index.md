---
title: 파드 및 서비스
description: 이스티오 지원 클러스터에서 실행할 쿠버네티스 파드 및 서비스를 준비한다.
weight: 40
keywords:
  - kubernetes
  - sidecar
...
- **Service association**: A pod must belong to at least one Kubernetes
  ...
  If a pod belongs to multiple [Kubernetes services](https://kubernetes.io/ko/docs/concepts/services-networking/service/),
...
- **`NET_ADMIN` and `NET_RAW` capabilities**: If your cluster enforces pod security policies,
  ...
  If you use the [Istio CNI Plugin](/ko/docs/setup/additional-setup/cni/),
...
- **Named service ports**: Service ports may optionally be named to explicitly specify a protocol.
  See [Protocol Selection](/ko/docs/ops/configuration/traffic-management/protocol-selection/) for
  more details.
...
If [pod security policies](https://kubernetes.io/ko/docs/concepts/policy/pod-security-policy/)
are [enforced](https://kubernetes.io/ko/docs/concepts/policy/pod-security-policy/#enabling-pod-security-policies)
...
[service account](https://kubernetes.io/ko/docs/tasks/configure-pod-container/configure-service-account/)
...
Otherwise, you will need to [provide the permission](https://kubernetes.io/ko/docs/concepts/policy/pod-security-policy/#authorizing-policies).
