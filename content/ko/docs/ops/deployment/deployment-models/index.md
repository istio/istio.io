---
title: 배포 모델
description: 이스티오 배포를 구성할 때 선택 사항과 고려 사항을 설명한다.
weight: 20
keywords:
  - single-cluster
  - multiple-clusters
...
이스티오의 프로덕션 배포를 구성할 때, 수많은 질문에 대답해야 한다.
메시가 싱글 {{< gloss "cluster" >}}클러스터{{< /gloss >}}로 제한될 것인가, 아니면 멀티 클러스터로 분산될 것인가?
모든 서비스가 완전히 연결된 싱글 네트워크에 배치될 것인가,
아니면 멀티 네트워크를 통해 서비스를 연결하기 위해 게이트웨이가 필요할 것인가?
클러스터 간에 잠재적으로 공유되는 싱글 {{< gloss "control plane" >}}컨트롤 플레인{{< /gloss >}}이 있는가,
아니면 고가용성(HA)을 보장하기 위해 멀티 컨트롤 플레인이 배포되는가?
모든 클러스터가 싱글 {{< gloss "multicluster" >}}멀티클러스터{{< /gloss >}} 서비스 메시에 연결될 것인가,
아니면 {{< gloss "multi-mesh" >}}멀티 메시{{< /gloss >}} 배포로 결합될 것인가?

이러한 모든 질문은 특히 이스티오 배포를 위한 독립적인 구성 차원을 나타낸다.
