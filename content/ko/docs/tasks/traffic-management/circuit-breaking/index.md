---
title: 회로 차단
description: 이 태스크는 연결, 요청, 이상치(outlier) 탐지를 위한 회로 차단 구성 방법을 보여 준다.
weight: 50
...
1.  Create a [destination rule](/ko/docs/reference/config/networking/destination-rule/) to apply circuit breaking settings
when calling the `httpbin` service:
    ...
    Otherwise requests will generate 503 errors as described [here](/ko/docs/ops/common-problems/network-issues/#503-errors-after-setting-destination-rule).
    {{< /warning >}}
...
governed by Istio.
    ...
    If you have enabled [automatic sidecar injection](/ko/docs/setup/additional-setup/sidecar-injection/#automatic-sidecar-injection), deploy the `fortio` service:
