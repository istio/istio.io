---
title: 요청 타임아웃
description: 이 태스크는 이스티오를 사용하여 엔보이에서 요청 타임아웃을 설정하는 방법을 보여 준다.
weight: 40
...
* Deploy the [Bookinfo](/ko/docs/examples/bookinfo/) sample application including the
  [default destination rules](/ko/docs/examples/bookinfo/#apply-default-destination-rules).
...
A timeout for HTTP requests can be specified using the *timeout* field of the [route rule](/ko/docs/reference/config/networking/virtual-service/#HTTPRoute).
...
If you examine the [fault injection task](/ko/docs/tasks/traffic-management/fault-injection/), you'll find out that the `productpage`
...
More details can be found [here](/ko/docs/concepts/traffic-management/#network-resilience-and-testing).
