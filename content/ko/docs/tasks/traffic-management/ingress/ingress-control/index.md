...
aliases:
    - /ko/docs/tasks/ingress.html
    - /ko/docs/tasks/ingress
owner: istio/wg-networking-maintainers
test: yes
---

Along with support for Kubernetes [Ingress](/ko/docs/tasks/traffic-management/ingress/kubernetes-ingress/), Istio offers another configuration model, [Istio Gateway](/ko/docs/reference/config/networking/gateway/). A `Gateway` provides more extensive customization and flexibility than `Ingress`, and allows Istio features such as monitoring and route rules to be applied to traffic entering the cluster.
...
In this case, you can access the gateway using the service's [node port](https://kubernetes.io/ko/docs/concepts/services-networking/service/#nodeport).
...
An ingress [Gateway](/ko/docs/reference/config/networking/gateway/) describes a load balancer operating at the edge of the mesh that receives incoming HTTP/TCP connections.
It configures exposed ports, protocols, etc.
but, unlike [Kubernetes Ingress Resources](https://kubernetes.io/ko/docs/concepts/services-networking/ingress/),
...
1.  Configure routes for traffic entering via the `Gateway`:
    ...
    You have now created a [virtual service](/ko/docs/reference/config/networking/virtual-service/)
    configuration for the `httpbin` service containing two route rules that allow traffic for paths `/status` and
    `/delay`.

    The [gateways](/ko/docs/reference/config/networking/virtual-service/#VirtualService-gateways) list
    ...
    [operations guide](/ko/docs/ops/common-problems/network-issues#route-rules-have-no-effect-on-ingress-gateway-requests)
    for more details.
    {{< /warning >}}
...
1.  If you have an external load balancer and it does not work for you, try to
    [access the gateway using its node port](/ko/docs/tasks/traffic-management/ingress/ingress-control/#determining-the-ingress-ip-and-ports).
