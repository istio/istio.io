---
title: 메시 시각화
description: 이 태스크는 이스티오 메시 내에서 서비스를 시각화하는 방법을 보여준다.
weight: 49
...
This task uses the [Bookinfo](/ko/docs/examples/bookinfo/) sample application as the example throughout. This task
assumes the Bookinfo application is installed in the `bookinfo` namespace.

## Before you begin

Follow the [Kiali installation](/ko/docs/ops/integrations/kiali/#installation) documentation to deploy Kiali into your cluster.
...
1.  To determine the Bookinfo URL, follow the instructions to determine the [Bookinfo ingress `GATEWAY_URL`](/ko/docs/examples/bookinfo/#determine-the-ingress-ip-and-port).
