---
title: 상호 TLS 마이그레이션
description: 이스티오 서비스를 점진적으로 상호 TLS로 마이그레이션하는 방법을 보여 준다.
weight: 40
...
Istio automatically configures workload sidecars to use [mutual TLS](/ko/docs/tasks/security/authentication/authn-policy/#auto-mutual-tls) when calling other workloads. By default, Istio configures the destination workloads using `PERMISSIVE` mode.
...
You can use the [Grafana dashboard](/ko/docs/tasks/observability/metrics/using-istio-dashboard/) to
...
* Understand Istio [authentication policy](/ko/docs/concepts/security/#authentication-policies) and related [mutual TLS authentication](/ko/docs/concepts/security/#mutual-tls-authentication) concepts.

* Read the [authentication policy task](/ko/docs/tasks/security/authentication/authn-policy) to
  learn how to configure authentication policy.

* Have a Kubernetes cluster with Istio installed, without global mutual TLS enabled (for example, use the `default` configuration profile as described in [installation steps](/ko/docs/setup/getting-started)).
...
We recommend you use [Istio Authorization](/ko/docs/tasks/security/authorization/authz-http/) to configure different paths with different authorization policies.
