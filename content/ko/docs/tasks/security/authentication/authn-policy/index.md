---
title: 인증 정책
description: 이스티오 인증 정책을 사용하여 상호 TLS 인증 및 기본 최종 사용자 인증을 설정하는 방법을 보여 준다.
weight: 10
...
the underlying concepts in the [authentication overview](/ko/docs/concepts/security/#authentication).

## Before you begin

* Understand Istio [authentication policy](/ko/docs/concepts/security/#authentication-policies) and related
[mutual TLS authentication](/ko/docs/concepts/security/#mutual-tls-authentication) concepts.

* Install Istio on a Kubernetes cluster with the `default` configuration profile, as described in
[installation steps](/ko/docs/setup/getting-started).
...
Also, for convenience, expose `httpbin.foo` via `ingressgateway` (for more details, see the [ingress task](/ko/docs/tasks/traffic-management/ingress/)).
...
Follow the instructions in
    [Determining the ingress IP and ports](/ko/docs/tasks/traffic-management/ingress/ingress-control/#determining-the-ingress-ip-and-ports)
