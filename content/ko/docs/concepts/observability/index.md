---
title: 관측성
description: 이스티오가 제공하는 텔레메트리 및 모니터링 기능을 설명한다.
weight: 40
...
- [**Metrics**](#metrics). Istio generates a set of service metrics based on the four "golden signals" of monitoring (latency, traffic, errors, and
  saturation). Istio also provides detailed metrics for the [mesh control plane](/ko/docs/ops/deployment/architecture/).
...
- [**Access Logs**](#access-logs). As traffic flows into a service within a mesh, Istio can generate a full record of each request, including source and
  ...
  [workload instance](/ko/docs/reference/glossary/#workload-instance) level.
...
The [Envoy documentation site](https://www.envoyproxy.io/ko/docs/envoy/latest/) includes a detailed overview of [Envoy statistics collection](https://www.envoyproxy.io/ko/docs/envoy/latest/intro/arch_overview/observability/statistics.html?highlight=statistics).
The operations guide on [Envoy Statistics](/ko/docs/ops/diagnostic-tools/proxy-cmd/) provides more information on controlling the generation of proxy-level metrics.
...
[dashboards](/ko/docs/tasks/observability/metrics/using-istio-dashboard/) for monitoring service behaviors based on these metrics.

The [standard Istio metrics](https://istio.io/v1.6/ko/docs/reference/config/policy-and-telemetry/metrics/) are
exported to [Prometheus](/ko/docs/ops/integrations/prometheus/) by default.
...
For more information on which metrics are maintained, please refer to the [reference documentation](/ko/docs/reference/commands/pilot-discovery/#metrics).
...
Istio supports a number of tracing backends, including [Zipkin](/ko/docs/tasks/observability/distributed-tracing/zipkin/),
[Jaeger](/ko/docs/tasks/observability/distributed-tracing/jaeger/), [Lightstep](/ko/docs/tasks/observability/distributed-tracing/lightstep/), and
...
{{< image link="/ko/docs/tasks/observability/distributed-tracing/zipkin/istio-tracing-details-zipkin.png" caption="Distributed Trace for a single request" >}}
...
logging. For more information, please refer to [Getting Envoy's Access Logs](/ko/docs/tasks/observability/logs/access-log/).
