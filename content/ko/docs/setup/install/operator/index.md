---
title: 이스티오 오퍼레이터 설치
description: 이스티오 오퍼레이터를 사용하여 쿠버네티스 클러스터에 이스티오를 설치하는 방법.
weight: 25
...
you can instead let the Istio [operator](https://kubernetes.io/ko/docs/concepts/extend-kubernetes/operator/)
...
The same [`IstioOperator` API](/ko/docs/reference/config/istio.operator.v1alpha1/) is used
to install Istio with the operator as when using the [istioctl install instructions](/ko/docs/setup/install/istioctl).
...
1. Install the [{{< istioctl >}} command](/ko/docs/ops/diagnostic-tools/istioctl/).
...
1. Deploy the Istio operator:
    ...
    See the [`istioctl operator init` command reference](/ko/docs/reference/commands/istioctl/#istioctl-operator-init) for details.
    ...
    Note that you need to [download the Istio release](/ko/docs/setup/getting-started/#download)
    to run the above command.
    {{< /tip >}}

## Install

To install the Istio `demo` [configuration profile](/ko/docs/setup/additional-setup/config-profiles/)
...
Refer to the [`IstioOperator` API](/ko/docs/reference/config/istio.operator.v1alpha1/#IstioOperatorSpec)
for the complete set of configuration settings.

## Canary Upgrade

You can use the operator to do a canary upgrade of an Istio control plane, the process is similar to the [canary upgrade with `istioctl`](/ko/docs/setup/upgrade/#canary-upgrades).
