---
title: 복제 컨트롤 플레인
description: 복제 컨트롤 플레인 인스턴스를 사용하여 여러 쿠버네티스 클러스터에 이스티오 메시를 설치한다.
weight: 2
...
[multicluster deployment](/ko/docs/ops/deployment/deployment-models/#multiple-clusters)
using multiple {{< gloss "primary cluster" >}}primary clusters{{< /gloss >}},
each with its own replicated [control plane](/ko/docs/ops/deployment/deployment-models/#control-plane-models),
...
* Authority to [deploy the Istio control plane](/ko/docs/setup/install/istioctl/)
...
1. Run the following commands in **every cluster** to deploy an identical Istio control plane
    ...
    * Create a Kubernetes secret for your generated CA certificates using a command similar to the following. See [Certificate Authority (CA) certificates](/ko/docs/tasks/security/cert-management/plugin-ca-cert/) for more details.
...
1. Export the `cluster2` gateway address:
    ...
    [Control Ingress Traffic](/ko/docs/tasks/traffic-management/ingress/ingress-control/#determining-the-ingress-ip-and-ports)
