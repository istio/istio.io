---
title: 구성 프로파일 설치
description: 내장형 이스티오 설치 구성 프로파일을 설명한다.
weight: 35
...
[installing Istio](/ko/docs/setup/install/istioctl/).
The profiles provide customization of the Istio control plane and of the sidecars for the Istio data plane.

You can start with one of Istio’s built-in configuration profiles and then further
[customize the configuration](/ko/docs/setup/install/istioctl/#customizing-the-configuration)
...
1. **default**: enables components according to the default settings of the
    ...
    [multicluster mesh](/ko/docs/ops/deployment/deployment-models/#multiple-clusters).
...
1. **demo**: configuration designed to showcase Istio functionality with modest resource requirements.
    It is suitable to run the [Bookinfo](/ko/docs/examples/bookinfo/) application and associated tasks.
    This is the configuration that is installed with the [quick start](/ko/docs/setup/getting-started/) instructions.

    {{< warning >}}
    This profile enables high levels of tracing and access logging so it is not suitable for performance tests.
    {{< /warning >}}

1. **minimal**: the minimal set of components necessary to use Istio's [traffic management](/ko/docs/tasks/traffic-management/) features.
...
To further [customize Istio and install addons](/ko/docs/setup/install/istioctl/#customizing-the-configuration), you can add one or more `--set <key>=<value>` options in the
`istioctl install` command that you use when installing Istio.
Refer to [integration](/ko/docs/ops/integrations) for more details.
