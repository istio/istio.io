...
We found that with 16 concurrent connections and 1000 RPS, Istio adds **3ms** over the baseline (P50) when a request travels through both a client and server proxy. At 64 concurrent connections, Istio adds **12ms** over the baseline, but when [policy and telemetry](/docs/concepts/policies-and-telemetry/) are disabled (`nomixer_both`), Istio only adds **7ms**.
