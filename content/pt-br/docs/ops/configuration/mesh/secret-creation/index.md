...
1. [`enableNamespacesByDefault` security configuration](/pt-br/docs/reference/config/installation-options/#security-options): default behavior if no labels are found on the `ServiceAccount`'s namespace

From these three values, the decision process mirrors that of the [`Sidecar Injection Webhook`](/pt-br/docs/ops/configuration/mesh/injection-concepts/). The detailed behavior is that:
