...
{{< tip >}}
The mesh authentication policy uses the [regular authentication policy API](/pt-br/docs/reference/config/security/istio.authentication.v1alpha1/)
...
To configure the client side, you need to set [destination rules](/pt-br/docs/concepts/traffic-management/#destination-rules) to use mutual TLS. It's possible to use
...
Also, for convenience, expose `httpbin.foo` via `ingressgateway` (for more details, see the [ingress task](/pt-br/docs/tasks/traffic-management/ingress/)).
