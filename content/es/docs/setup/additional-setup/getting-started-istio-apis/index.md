...
This guide lets you quickly evaluate Istio, using only its legacy APIs. If you want to use the Kubernetes Gateway API, [please see that example](/es/docs/setup/getting-started/).
...
[supported version](/es/docs/releases/supported-releases#support-status-of-istio-releases) of Kubernetes ({{< supported_kubernetes_versions >}}). You can use any supported platform, for
example [Minikube](https://kubernetes.io/docs/tasks/tools/install-minikube/) or
others specified by the
[platform-specific setup instructions](/es/docs/setup/platform-setup/).
...
1.  For this installation, we use the `demo`
    [configuration profile](/es/docs/setup/additional-setup/config-profiles/). It's
    ...
    [platform instructions](/es/docs/setup/platform-setup/) for details.
    {{< /warning >}}
...
[Istio Ingress Gateway](/es/docs/concepts/traffic-management/#gateways), which maps a path to a
...
Istio integrates with [several](/es/docs/ops/integrations) different telemetry applications. These can help you gain
