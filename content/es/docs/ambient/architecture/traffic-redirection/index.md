...
Una vez que se notifica al agente de nodo `istio-cni` que se debe agregar un pod a el mesh (ya sea desde el complemento CNI, si el pod es nuevo, o desde el servidor de la API de Kubernetes, si el pod ya se está ejecutando pero necesita ser agregado), se realiza la siguiente secuencia de operaciones:
...
- Una vez que las reglas de redirección en el pod están en su lugar y el ztunnel ha establecido los puertos de escucha, el pod se agrega a el mesh y el tráfico comienza a fluir a través del ztunnel local del nodo.

El tráfico hacia y desde los pods en el mesh se cifrará completamente con mTLS de forma predeterminada.

Los datos ahora entrarán y saldrán del namespace de red del pod cifrados. Cada pod en el mesh tiene la capacidad de hacer cumplir la política de el mesh y cifrar el tráfico de forma segura, aunque la aplicación de usuario que se ejecuta en el pod no tiene conocimiento de ninguna de las dos cosas.

Este diagrama ilustra cómo fluye el tráfico cifrado entre los pods en el mesh ambient en el nuevo modelo:
