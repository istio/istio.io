...
Una vez que hayas agregado aplicaciones a el mesh ambient, la superposición segura L4 de ambient tunelizará el tráfico entre tus pods a través del puerto 15008. Una vez que el tráfico seguro ingrese al pod de destino con un puerto de destino de 15008, el tráfico se redirigirá al puerto de destino original.
...
Este comportamiento se habilita de forma transparente cuando agregas pods a el mesh ambient, y de forma predeterminada, ambient usa la dirección de enlace local `169.254.7.127` para identificar y permitir correctamente los paquetes de sondeo de salud de kubelet.

Sin embargo, si tu carga de trabajo, namespace o cluster tiene una `NetworkPolicy` de entrada o salida preexistente, dependiendo del CNI que estés utilizando, los paquetes con esta dirección de enlace local pueden ser bloqueados por la `NetworkPolicy` explícita, lo que hará que las sondas de salud de tu pod de aplicación comiencen a fallar cuando agregues tus pods a el mesh ambient.
