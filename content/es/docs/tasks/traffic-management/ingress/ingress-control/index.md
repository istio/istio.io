...
*   Inicie la muestra [httpbin]({{< github_tree >}}/samples/httpbin), que servirá como service de destino
    ...
    la inyección de sidecar habilitada (es decir, el service de destino puede estar dentro o fuera de el mesh de Istio).

## Configuración de la entrada usando un gateway

Un `Gateway` de entrada describe un balanceador de carga que opera en el borde de el mesh y que recibe conexiones HTTP/TCP entrantes.
...
{{< warning >}}
Las solicitudes internas de otros services en el mesh no están sujetas a estas reglas
