...
[Grafana](/es/docs/ops/integrations/grafana/) and [Kiali](/es/docs/tasks/observability/kiali/).
...
`--set meshConfig.enablePrometheusMerge=false` during [installation](/es/docs/setup/install/istioctl/).
...
* For application stats, if [Strict mTLS](/es/docs/tasks/security/authentication/authn-policy/#globally-enabling-istio-mutual-tls-in-strict-mode)
...
However, the application metrics will follow whatever [Istio authentication policy](/es/docs/tasks/security/authentication/authn-policy) has been configured
for the workload.
...
See [Understanding TLS Configuration](/es/docs/ops/configuration/traffic-management/tls-configuration/) for more information.
{{< /tip >}}
...
and deploy it with [sidecar injection](/es/docs/setup/additional-setup/sidecar-injection/).
...
See [Using Prometheus for production-scale monitoring](/es/docs/ops/best-practices/observability/#using-prometheus-for-production-scale-monitoring)
