...
For a high level overview, see [Istio Architecture](/es/docs/ops/deployment/architecture/).
Note that Istio deployments are highly flexible; below, we will primarily assume the worst case scenarios.

### Istiod

Istiod serves as the core control plane component of Istio, often serving the role of the [XDS serving component](/es/docs/concepts/traffic-management/) as well
as the mesh [mTLS Certificate Authority](/es/docs/concepts/security/).
...
Istio can optionally be deployed with the [Istio CNI Plugin `DaemonSet`](/es/docs/setup/additional-setup/cni/).
...
Istio may [optionally](/es/docs/overview/data plane-modes/) deploy a sidecar proxy next to an application.
...
Any applicable [policies](/es/docs/tasks/security/authorization/) for the workload will be enforced before forwarding the traffic to the workload.

However, Istio does not currently guarantee that _outgoing_ traffic is redirect to the proxy.
See [traffic capture limitations](/es/docs/ops/best-practices/security/#understand-traffic-capture-limitations).
As such, care must be taken to follow the [securing egress traffic](/es/docs/ops/best-practices/security/#securing-egress-traffic) steps if outbound policies are required.

## Mutual TLS Properties

[Mutual TLS](/es/docs/concepts/security/#mutual-tls-authentication) provides the basis for much of Istio's security posture.
...
{{< tabset category-name="data plane" >}}
{{< tab name="Sidecar mode" category-value="sidecar" >}}
In sidecar mode, the client sidecar will [automatically use TLS](/es/docs/ops/configuration/traffic-management/tls-configuration/#auto-mtls) when connecting to a service
that is detected to support mTLS. This can also be [explicitly configured](/es/docs/ops/configuration/traffic-management/tls-configuration/#sidecars).
Note that this automatic detection relies on Istio associating the traffic to a Service.
[Unsupported traffic types](/es/docs/ops/configuration/traffic-management/traffic-routing/#unmatched-traffic) or [configuration scoping](/es/docs/ops/configuration/mesh/configuration-scoping/) can prevent this.

When [connecting to a backend](/es/docs/concepts/security/#secure-naming), the set of allowed identities is computed, at the Service level, based on the union of all backend's identities.
{{< /tab >}}
...
* [Observability](/es/docs/tasks/observability/) features can be used to identify the attack.
* [Policies](/es/docs/tasks/security/authorization/) can be used to restrict what type of traffic a workload can send or receive.
...
Istio offers the flexibility of [configuring the granularity of a waypoint deployment](/es/docs/ambient/usage/waypoint/#useawaypoint).
...
Following the [security best practices](/es/docs/ops/best-practices/security) is crucial to maintaining a secure cluster.
