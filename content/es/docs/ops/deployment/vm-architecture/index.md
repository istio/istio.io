...
Before reading this document, be sure to review [Istio's architecture](/es/docs/ops/deployment/architecture/) and [deployment models](/es/docs/ops/deployment/deployment-models/).
...
In this mesh, there is a single [network](/es/docs/ops/deployment/deployment-models/#network-models), where pods and virtual machines can communicate directly with each other.
...
In this mesh, there are multiple [networks](/es/docs/ops/deployment/deployment-models/#network-models), where pods and virtual machines are not able to communicate directly with each other.
...
* [`WorkloadGroup`](/es/docs/reference/config/networking/workload-group/) represents a logical group of virtual machine workloads that share common properties. This is similar to a `Deployment` in Kubernetes.
* [`WorkloadEntry`](/es/docs/reference/config/networking/workload-entry/) represents a single instance of a virtual machine workload. This is similar to a `Pod` in Kubernetes.
...
Once a virtual machine has been [configured and added to the mesh](/es/docs/setup/install/virtual-machine/#configure-the-virtual-machine), a corresponding `WorkloadEntry` will be automatically created by the Istio control plane.
For example:
...
For virtual machine expansion, Istio provides similar functionality via a [DNS Proxy](/es/docs/ops/configuration/traffic-management/dns-proxy/).
