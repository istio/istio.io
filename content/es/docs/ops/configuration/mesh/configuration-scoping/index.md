...
These are then sent to the data plane (see [Architecture](/es/docs/ops/deployment/architecture/) for more information).
...
The [`egress.hosts`](/es/docs/reference/config/networking/sidecar/#IstioEgressListener) field in `Sidecar`
...
While the previous controls operate on a workload or service owner level, [`DiscoverySelectors`](/es/docs/reference/config/istio.mesh.v1alpha1/#MeshConfig) provides mesh wide control over configuration visibility.
...
[Headless services](https://kubernetes.io/docs/concepts/services-networking/service/#headless-services) (besides ones declared as [HTTP](/es/docs/ops/configuration/traffic-management/protocol-selection/#explicit-protocol-selection))
...
so it will be treated as [Unmatched traffic](/es/docs/ops/configuration/traffic-management/traffic-routing/#unmatched-traffic).

### What about Gateways?

While [Gateways](/es/docs/setup/additional-setup/gateway/) will respect `exportTo` and `DiscoverySelectors`, `Sidecar` objects do not impact Gateways.
