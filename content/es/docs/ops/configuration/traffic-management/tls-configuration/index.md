...
Refer to [TLS configuration mistakes](/es/docs/ops/common-problems/network-issues/#tls-configuration-mistakes)
...
1. **External inbound traffic**
    ...
    The mTLS mode is configured using a [`PeerAuthentication` resource](/es/docs/reference/config/security/peer_authentication/).
...
1. **Local outbound traffic**
    ...
    If [automatic protocol selection](/es/docs/ops/configuration/traffic-management/protocol-selection/#automatic-protocol-selection)
    ...
    [manually specify the protocol](/es/docs/ops/configuration/traffic-management/protocol-selection/#explicit-protocol-selection).
...
1. **External outbound traffic**
    ...
    [`DestinationRule` resource](/es/docs/reference/config/networking/destination-rule/).
...
This is done based on the server configuration in a [`Gateway` resource](/es/docs/reference/config/networking/gateway/).
...
you will end up with [double encryption](/es/docs/ops/common-problems/network-issues/#double-tls).
This works, but is often not the desired behavior.

A `VirtualService` bound to the gateway needs care as well to
[ensure it is consistent](/es/docs/ops/common-problems/network-issues/#gateway-mismatch)
