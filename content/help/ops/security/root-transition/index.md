...
You may choose to upgrade to 1.x.8 (_x_ can be 0 or 1, depending on your current version) or
later versions to avoid this problem in the future.
...
1. Upgrade to Istio 1.x.8:

    {{< warning >}}
    To ensure the control plane components and Envoy sidecars all load the new certificates and keys, this step is mandatory.
    {{< /warning >}}

    Upgrade your control plane and `istio-proxy` sidecars to 1.x.8.
    To upgrade to 1.x.8, please follow the Istio [upgrade procedure](/docs/setup/kubernetes/upgrade/steps/).
...
### Can I upgrade to 1.x.8 first, and then do the root transition?

Yes, you can. You can upgrade to 1.x.8 as normal.
...
Please make sure you have updated to Istio 1.x.8 for the `istio-proxy` sidecars in Step 4.
...
This may happen if you did not upgrade to 1.x.8.
