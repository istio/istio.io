...
One workaround is to remove the proxy settings from the `kube-apiserver` manifest and restart the server or use a later version of Kubernetes, another workaround is to include istio-sidecar-injector.istio-system.svc in the no_proxy value.
