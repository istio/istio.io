...
One workaround is to remove the proxy settings from the `kube-apiserver` manifest, another workaround is to include `istio-sidecar-injector.istio-system.svc` in the `no_proxy` value. Make sure that `kube-apiserver` is restarted after each workaround.
