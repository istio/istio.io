...
5. 创建一个 `collection` 来保存评级数据。以下命令将两个评级都设置为 `1`，以便在 Bookinfo _ratings_ service 使用数据库时提供视觉验证（默认 Bookinfo _ratings_
   为 `4` 和 `5`）
...
1.  [开启 Envoy’s访问日志](/zh/docs/tasks/observability/logs/access-log/#enable-envoy-s-access-logging)
...
1.  [开启 Envoy’s 访问日志](/zh/docs/tasks/observability/logs/access-log/#enable-envoy-s-access-logging)
