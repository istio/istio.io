...
新的版本经常被推出，并且需要根据用户请求快速扩展和缩减。
随着现代应用程序需要通过不依赖主机托管的方式来获得资源利用效率，
...
您将（希望）获得身份证明文件。您可以将该身份证明文件交给其他人，
...
对应用程序之间传输的数据进行加密至关重要 - 因为在一个违规行为屡见不鲜、违规成本高昂但实际效果微不足道的世界中，
...
以便只有源和目标才能解密数据。此外，您可能需要自定义加密，例如根据您安全团队的要求选择特定的密码。

### 完整性  {#integrity}

通过网络从源发送到目标的加密数据一旦发送，就不能被除源和目标之外的任何身份进行修改。
换句话说，接收到的数据与发送的数据相同。如果您不具有[数据完整性](https://en.wikipedia.org/wiki/Data_integrity)，
那么中间人可能会在源和目标之间的通信过程中修改数据的某些位或整个内容。

### 访问策略执行  {#access-policy-enforcement}

应用程序所有者需要将访问策略实施于应用程序中，并正确、一致且明确地执行这些策略。
为了对通信通道的两端实施策略，我们需要为每一端提供一个应用程序标识。
...
或者只有具有特定 JWT 颁发者颁发的 JWT 令牌的应用程序才能调用 `checkout` 应用程序的 `GET` 方法的场景。
...
与联邦政府合作时需要这样做。为了遵循美国政府制定的有关网络安全的指导方针，
许多私营部门自愿使用这些 FIPS 标准。
...
1. 保密性：数据经过加密，只有客户端和服务器可见。
   数据必须使用密钥进行明确加密并绑定到源和目标身份文档中，以便可靠地保护应用层流量。
1. 完整性：通过通道发送的数据在未经检测的情况下无法被修改。
   这是通过以下事实来保证的，只有源和目标具有加密和解密给定会话数据的密钥。

### mTLS 内部  {#mtls-internals}

我们已经确定，可加密验证的身份是保护通道安全和支持访问策略执行的关键，
并且我们已经确定，mTLS 是一种经过实战检验的协议，
它要求在通道两端使用可加密验证的身份提供一些极其重要的保证 - 让我们详细了解 mTLS 协议的实际工作原理。
...
1. 第 2 阶段：此阶段及之后的所有内容均被加密。在此阶段，`frontend` 和 `checkout` 建立其他握手参数，
   以及客户端是否也经过身份验证 - 即 mTLS。
1. 第 3 阶段：`frontend` 通过其可加密验证的身份验证 `checkout`
   （并且，在 mTLS 中，`checkout` 以相同的方式验证 `frontend`）。

自 TLS 1.2 以来，与握手相关的内容存在一些主要差异，
...
使用与握手中完全相同的协商参数来加密流量以确保流量的保密性和完整性至关重要（并且是规范的一部分）。
...
它们通常由[证书颁发机构（CA）](https://en.wikipedia.org/wiki/Certificate_authority)颁发，
该机构拥有自己的[根证书](https://en.wikipedia.org/wiki/Root_certificate)或使用来自其根 CA 的中间证书。
...
您的路线就符合 [FIPS 140-2 或 140-3 验证](https://csrc.nist.gov/projects/cryptographic-module-validation-program/validated-modules)。

## 可能会出现什么问题  {#what-could-go-wrong}

严格按照 TLS 1.3 规范的规定实施 mTLS 至关重要。
如果不使用符合 TLS 规范的正确 mTLS，以下情况可能会在未检测到的情况下出现问题：

### 如果连接中间有人悄悄捕获加密数据怎么办？  {#what-if-someone-in-the-middle-of-the-connection-silently-captures-the-encrypted-data}
...
### 如果源身份或目标身份在加密上不安全怎么办？  {#what-if-either-source-or-destination-identity-is-not-cryptographically-secure}
...
地址被重复使用和身份错误的情况，因为策略未正确执行。
例如，如果您在对等点之间的连接上没有加密身份，
您的系统将必须从身份缓存中获取身份，该身份可能已过时或不完整。

这些将身份映射到工作负载 IP 的身份缓存不是
[ACID](https://en.wikipedia.org/wiki/ACID)（原子性、一致性、隔离性和持久性），
...
让我们假设以下大规模多集群部署来说明身份缓存的陈旧问题：

1. 100 个集群，每个集群有 100 个节点，每个节点有 20 个 Pod。Pod 总数为 200,000 个。
1. 0.25% 的 Pod 始终处于搅动状态（滚动、重启、恢复、节点搅动等），每次搅动都是 10 秒的窗口期。
...
将根证书的私钥保存在保持[离线](https://en.wikipedia.org/wiki/Online_and_offline)的 HSM
