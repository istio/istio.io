---
title: "简化到通配符目标的出站路由"
description: "Istio 现在支持带有 DYNAMIC_DNS 解析的通配符 ServiceEntry，允许 Sidecar 将流量直接路由到通配符 HTTPS 目标，同时简化出站配置。"
...
## 为什么通配符 HTTPS 出站难以实现 {#why-wildcard-https-egress-is-difficult}
...
## 通过出站网关进行 SNI 路由 {#sni-routing-via-egress-gateway}

这个问题之前在 Istio 博客文章[将出站流量路由到通配符目标](/zh/blog/2023/egress-sni/)中讨论过。
该架构包含一个专用的出站网关设置，用作 SNI 转发代理。

{{< image width="90%" link="./egress-sni-flow.svg" alt="使用任意域名进行出站 SNI 路由" title="使用任意域名进行出站 SNI 路由" caption="应用程序 → Sidecar → 出站网关 → SNI 检测 → 外部目标" >}}

上图最初发表于[将出站流量路由到通配符目的地](/zh/blog/2023/egress-sni/)。

如上图所示：

1. 应用程序发起 HTTPS 连接。
1. Sidecar 代理拦截此连接，并向出站网关发起内部 mTLS 连接。
1. 网关终止此内部 mTLS 连接。
...
* `ServiceEntry` 和 `VirtualService` 用于将通配符域名流量转发到出站网关。
* `DestinationRule` 用于边车服务器和网关之间的 mTLS 通信。
* `EnvoyFilter` 配置使出站网关能够执行动态 SNI 转发，这是该解决方案中最复杂的部分。
...
使 Sidecar 代理能够直接路由通配符出站 TLS 流量，而无需出站网关。
...
### Ambient 模式下的出站流量 {#egress-traffic-in-ambient-mode}
...
* 一个 `Sidecar` 资源，它将评分服务的出站主机限制在 `istio-system`
...
* [将出站流量路由到通配符目标](/zh/blog/2023/egress-sni/)
