...
将每一个集群都设置为{{< gloss >}}primary cluster{{< /gloss >}}。
两个集群都运行在网络 `network1` 上，这意味着两个集群中的 Pod 可以直接通信。

继续之前，确保完成了[准备工作](/zh/docs/setup/install/multicluster/before-you-begin)中的步骤。

在此配置中，每一个控制平面均监测两个集群 API 服务器的服务端点。

服务的工作负载跨集群边界直接通讯（pod 到 pod）。
...
## 将 `cluster1` 设置为主集群 {#configure-cluster1-as-a-primary}
...
## 将 `cluster2` 设置为主集群 {#configure-cluster2-as-a-primary}
