---
title: OpenTelemetry
description: 本任务告诉您如何配置 Envoy 代理发送访问日志 OpenTelemetry 收集器。
weight: 10
...
## 启用 Envoy 的访问日志{#enable-envoys-access-logging}

要启用访问日志记录，请使用 [Telemetry API](/zh/docs/tasks/observability/telemetry/)。

编辑 `MeshConfig`，以添加一个名为 `otel` 的 OpenTelemetry 提供者，这涉及添加扩展提供程序节：
...
如果您使用 `IstioOperator` CR 安装 Istio，下面需要将字段添加到您的配置中：
...
| 日志 Operator| sleep 访问日志| httpbin 访问登录 |
|--------------|---------------------|-----------------------|
...
### 禁止特使的访问日志{#disable-envoys-access-logging}
