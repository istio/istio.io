...
1. 遵循位于 [Integrations](/zh/docs/ops/integrations/) 下的追踪安装指南，根据您喜欢的追踪后端安装适当的插件和配置您的 Istio 代理以将追踪发送到追踪部署。
...
1. 在spans中添加自定义标签。这些标签可以基于静态文字添加请求标头中的值、环境值或字段。这可以用来在特定于您的环境的跨度中注入其他信息。
...
1.  在从工作负载代理环境变量填充自定义标记的值时，可以使用环境变量。
    ...
    {{< warning >}}
    为了添加基于环境变量的自定义标记，您必须修改根 Istio 系统名称空间中的`istio-sidecar-injector` ConfigMap
    {{< /warning >}}
