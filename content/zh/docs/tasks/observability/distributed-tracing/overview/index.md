...
Istio 利用 [Envoy 的分布式追踪](https://www.envoyproxy.io/docs/envoy/v1.12.0/intro/arch_overview/observability/tracing)功能提供了开箱即用的追踪集成。确切地说，Istio 提供了安装各种追踪后端服务的选项，并且通过配置代理来自动发送追踪 Span 到分布式追踪系统服务。
请参阅 [Zipkin](/zh/docs/tasks/observability/distributed-tracing/zipkin/)、[Jaeger](/zh/docs/tasks/observability/distributed-tracing/jaeger/)、[Lightstep](/zh/docs/tasks/observability/distributed-tracing/lightstep/) 和 [OpenCensus Agent](/zh/docs/tasks/observability/distributed-tracing/opencensusagent/) 的任务文档来了解 Istio 如何与这些分布式追踪系统一起工作。

## 追踪上下文传递{#trace-context-propagation}

尽管 Istio 代理能够自动发送 Span，但需要一些附加信息才能将这些 Span 加到同一个调用链。
所以当代理发送 Span 信息的时候，应用程序需要附加适当的 HTTP 请求头信息，这样才能够把多个 Span 加到同一个调用链。
