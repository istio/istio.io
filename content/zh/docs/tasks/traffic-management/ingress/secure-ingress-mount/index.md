...
### 为 `bookinfo.com` 创建服务器证书和私钥{#create-a-server-certificate-and-private-key-for-bookinfo-com}
...
### 使用新证书重新部署 `istio-ingressgateway`{#redeploy-istio-ingressgateway-with-the-new-certificate}
...
### 配置 `bookinfo.com` 主机的流量{#configure-traffic-for-the-bookinfo-com-host}
...
1. 删除 `Gateway` 配置、`VirtualService` 和 secres：
