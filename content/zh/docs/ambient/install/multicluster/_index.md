...
{{< warning >}}
虽然 Ambient **多网络多集群**已达到 Beta 测试阶段并被认为可用于生产环境，
但其部署仍存在一些已知限制。我们计划在未来的 Istio 版本中解决这些限制，
同时，请查看以下列表，确认这些已知限制是否适用于您的用例。
{{< /warning >}}
...
- 仅支持统一的服务范围配置 - 所有集群的服务范围必须一致
- 仅使用本地集群的服务范围配置作为真实来源
- 远程集群服务的作用域未得到有效保护，这可能导致同一服务在不同集群中具有不同作用域时出现意外的流量行为。
- 跨集群服务发现可能不遵循预期的服务边界

如果服务的 waypoint 被标记为全局，则该服务也将是全局的：

- 这可能会导致单网络多集群部署中出现意想不到的跨集群流量。
...
- 当故障转移到远程网络时，由于 HTTP 请求的多路复用和连接池机制，远程网络上的单个端点可能会收到不成比例的请求数量。
- 目前 Sidecar 模式下也存在非常类似的问题。
