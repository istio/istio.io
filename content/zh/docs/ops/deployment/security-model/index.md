...
### K8s 账户被盗用 {#k8s-account-compromise}

在这种情况下，具有部署 Istio 资源 (``networking.istio.io/v1``) 权限的 Kubernetes 帐户被攻破。

此类被攻破的账户使攻击者能够控制资源部署，从而管理服务网格中的流量。
虽然流量管理 CRD 是**命名空间资源**，但它们会影响来自命名空间外部服务的流量路由行为，进而影响服务网格的安全性。

例如，使用[网格网关选项](/zh/docs/reference/config/networking/virtual-service/#VirtualService-gateways)部署的
`VirtualService` 会在服务网格中启用中间人攻击。攻击者控制的服务可以：

* 作为目标服务。
* 将流量重定向到备用目标。
* 丢弃请求以中断通信（拒绝服务攻击）。

虽然攻击者可以尝试将流量转发到目标服务以读取或修改两个服务之间的通信，
但他们可能会失败，因为他们无法绕过 Istio 的 [Layer 4 和 Layer 7 安全功能](/zh/docs/overview/dataplane-modes/#layer-4-vs-layer-7-features)，
具体取决于网格配置。

更多详情和可能的缓解措施可以在[关于**基于命名空间的多租户 Istio CRD 的安全注意事项**的博客文章](/zh/blog/2026/security-considerations-on-namespace-based-multi-tenancy/)中找到。
