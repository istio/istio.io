...
它可以针对运行的群集或一组本地配置文件运行。它还可以将两者结合起来使用，
...
`kubectl get virtualservice ratings` 会给你类似这样的结果：
...
由于您无权更新命名空间，因此无法通过注释命名空间来解析消息。相反，
...
您也可以使用资源上的注释忽略特定的分析器消息。例如，要忽略资源
...
- **此工具针对的是哪个 Istio 版本？**
    ...
    就目前而言，analysis 是向后兼容的，所以你可以在运行 Istio 1.1 的集群上使用 1.4 版本的
   `istioctl analyze`，并且会得到有用的反馈。对老版本 Istio 没有意义的分析规则将被跳过。

    如果你决定使用最新的 `istioctl` 来对一个运行老版本 Istio 的集群进行分析，
