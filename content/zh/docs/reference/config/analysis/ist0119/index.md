...
当认证策略指定使用 JWT 认证但目标
[Kubernetes 服务](https://kubernetes.io/zh-cn/docs/concepts/services-networking/service/)配置不正确时，
会出现此消息。正确定位到 Kubernetes 服务需要使用 http|http2|https 前缀来命名端口
（请参见[协议选择](/zh/docs/ops/configuration/traffic-management/protocol-selection/)），
并且还需要使用 TCP 协议；协议留空也可以，因为其默认值就是 TCP。
...
- JWT 认证只支持 http、https 或 http2。将 Service
  的端口名重命名为 `<http|https|http2>[-<suffix>]` 这种格式即可。
