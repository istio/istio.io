---
title: Heml 安装参数变动表
description: 本文详细介绍了 Istio 1.2 系列到 Istio 1.3 系列之间的惨状参数变化详情。
weight: 30
...
| `global.proxy.envoyAccessLogService.host` | `` | 列如：accesslog-service.istio-system` |
| `global.proxy.envoyAccessLogService.port` | `` | 列如：15000 |
...
| `global.proxy.protocolDetectionTimeout` | `10ms` | 在服务端，自动协议检测使用一组启发式方法来确定连接是否正在使用 `TLS` ，以及所使用的应用协议（例如，http vs tcp）。 这些试探法依赖于客户端发送第一次请求数。对于一些优先发现的协议，如 MySQL协议，MongoDB协议等等，`Envoy`在完成协议检测超时情况下，默认为非`mTLS`纯 `TCP`流量。 设置此字段可调整`Envoy`等待客户端发送第一次请求数据时间。 （必须> = 1ms） |
| `global.proxy.enableCoreDumpImage` | `ubuntu:xenial` | 当 `enableCoreDump` 设置为 `true` 的时候，启动核心存储的镜像 |
| `global.defaultTolerations` | `[]` | 默认`tolerations` 节点 将应用于所有部署，以便可以将所有 `Pod` 调度到匹配  `taints`的节点。每个组件都可以通过在下面的相关部分中添加其 `tolerations block`并设置所需的值来覆盖这些默认值。 如果期望将 Istio 控制平面的所有`Pod` 调度到具有指定 `taints` 特性的特定节点，请配置此字段。 |
| `global.meshID` | `""` | `MeshID`表示 `Mesh`标识符。在 `Mesh`将彼此交互的范围内，它应该是唯一的，但是并不需要是全局/全局唯一的。例如，如果满足以下任一条件，则两个 Mesh必须具有不同的 `MeshID`，`Mesh`将在一个位置聚集其遥测 `Mesh`将联合在一起-将编写一个策略，引用另一个 `Mesh`管理员希望这些条件中的任何一种将来都可能成为现实，因此应确保为其 `Mesh`分配了不同的 `MeshID`。在多群集 `Mesh`中，每个群集必须（手动或自动）配置为具有相同的 `MeshID`值。如果现有群集“加入”多群集`Mesh`则需要将其迁移到新的 `MeshID`。迁移 TBD的详细信息，并且在安装后更改 `MeshID`可能是一项破坏性操作。如果 `Mesh`管理者未指定值，则 Istio将使用 `Mesh`的“信任域”的值。最佳实践是选择适当的“信任域”值。 |
...
| `mixer.telemetry.reportBatchMaxEntries` | `100` | 将`reportBatchMaxEntries`设置为0表示使用默认的批处理请求数目（即每次处理100个请求）。 正值表示遥测数据发送到 `mixer` 服务器之前已批处理的请求数 |
| `mixer.telemetry.reportBatchMaxTime` | `1s` | 将`reportBatchMaxTime`设置为0以使用默认的批处理请求时间（即每1秒一次批量请求）。 时间大于零表示处理完上次请求后并将遥测数据发送到 `mixer` 服务器 ，进行下次批处理前的最大等待时间 |
...
| `security.enableNamespacesByDefault` | `true` | 如果在给定名称空间上找不到 `ca.istio.io/env` 或 `ca.istio.io/override` 标签，则确定 Citadel默认行为。 例如：考虑一个名为 “ target” 的名称空间，该名称空间既没有 `ca.istio.io/env` 也没有 `ca.istio.io/override` 名称空间标签，为了确定是否为在 “ target” 名称空间中创建的服务帐户生成机密，Citadel 将采用此选项。 如果在这种情况下此选项的值为“ true”，则将为 “ target” 名称空间生成秘密。 如果此选项的值为“ false”，则 Citadel在创建服务帐户时不会生成机密信息。 |
...
| `pilot.enableProtocolSniffing` | `false` | 如果启用了协议嗅探。 默认为false。|

## 删除配置选项｛#removed-configuration-options｝
