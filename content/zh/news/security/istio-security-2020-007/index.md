...
* 对于 1.5.x 部署： 请升级至  [Istio 1.5.7](/news/releases/1.5.x/announcing-1.5.7) 或更高的版本。
* 对于 1.6.x 部署： 请升级至  [Istio 1.6.4](/news/releases/1.6.x/announcing-1.6.4) 或更高的版本。
...
1. 通过下载 [custom-bootstrap-runtime.yaml](/news/security/istio-security-2020-007/custom-bootstrap-runtime.yaml)创建配置映射。 根据部署中各个网关实例所需的并发连接数，在配置映射中更新 `global_downstream_max_connections` 。 一旦达到限制，Envoy 将开始拒绝 TCP 连接。

    {{< text bash >}}
    $ kubectl -n istio-system apply -f custom-bootstrap-runtime.yaml
    {{< /text >}}

1. 使用以上配置来修补 Ingress 网关。 下载 [gateway-patch.yaml](/news/security/istio-security-2020-007/gateway-patch.yaml) 文件并使用如下命令。
