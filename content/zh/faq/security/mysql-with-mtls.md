...
您可能会发现安装 Istio 后 MySQL 无法连接。这是因为在 `demo` [configuration profile](/zh/docs/setup/additional-setup/config-profiles/)中默认使用的 `PERMISSIVE` 模式不适用于 MySQL。您可能会看到类似于 `ERROR 2013 (HY000): Lost connection to MySQL server at
'reading initial communication packet', system error: 0` 的错误。
