...
[Авторизація](/docs/concepts/security/#authorization) відіграє важливу роль у безпеці Istio. Потрібно докласти зусиль для налаштування правильних політик авторизації для найкращого захисту кластерів. Важливо розуміти наслідки цих конфігурацій, оскільки Istio не може визначити правильну авторизацію для всіх користувачів. Будь ласка, дотримуйтесь цієї частини документації повністю.

### Моделі безпечної політики авторизації {#safer-authorization-policy-patterns}

#### Використовуйте підхід "стандартно заборонено" {#use-default-deny-patterns}
...
Використовуйте підходи `ALLOW-with-positive-matching` або `DENY-with-negative-match` за можливості. Ці політики авторизації є безпечнішими, оскільки найгірший результат у разі помилки політики — це несподівана відмова від обслуговування (помилка 403), а не обхід політики авторизації.
...
Перевірка сертифіката, наданого сервером, у CA є недостатньою, оскільки також потрібно перевірити альтернативні імена субʼєктів.
...
### Обмеження прав на створення `Gateway` {#restrict-gateway-creation-privileges}
...
Istiod відкриває кілька портів для налагодження та моніторингу. Стандартно точки налагодження тепер вимагають автентифікації:
...
- Порт `15010` відкриває службу XDS через gRPC у відкритому тексті. Точки налагодження XDS (`syncz`, `config_dump`) на цьому порту стандартно вимагають автентифікації, що фактично блокує доступ у відкритому тексті, коли вона ввімкнена. Використовуйте порт 15012 (TLS) для автентифікованого доступу до налагодження XDS. Саму службу XDS у відкритому тексті можна відключити, додавши прапорець `--grpcAddr=""` до розгортання istiod. Примітка: дуже чутливі сервіси, такі як сервіси підпису та розповсюдження сертифікатів, ніколи не обслуговуються у відкритому тексті.
- Порт `15012` відкриває службу XDS через TLS/mTLS gRPC (рекомендовано для промислового використання). Точки налагодження XDS доступні через цей порт з автоматичною автентифікацією mTLS.
- Порт `15014` відкриває точки налагодження (`/debug/syncz`, `/debug/registryz`, `/debug/config_dump` тощо) через HTTP (відкритий текст). Ці точки стандартно вимагають автентифікації за допомогою токенів службових облікових записів з аудиторією `istio-ca`.

Автентифікація точок налагодження контролюється змінною середовища `ENABLE_DEBUG_ENDPOINT_AUTH` (стандартно увімкнена). Коли вона ввімкнена, авторизація на основі просторів імен обмежує несистемні простори імен конкретними точками (`config_dump`, `ndsz`, `edsz`) лише для проксі з того самого простору імен. Щоб вимкнути автентифікацію та відновити застарілу поведінку, встановіть `ENABLE_DEBUG_ENDPOINT_AUTH=false` для istiod.

Див. [Інтеграційний посібник](/docs/ops/integrations/integration-guide/debug-endpoints/) для деталей щодо доступу до точок налагодження з інтеграцій.

### Панель даних {#data-plane}

Проксі відкриває різні порти. Зовні доступні порт `15090` (телеметрія), порт `15021` (перевірка стану) та порт `15020` (обʼєднана телеметрія Prometheus від агента Istio, Envoy та застосунку). Порт `15000` надає точки налагодження і відкритий лише через `localhost`. Як результат, застосунки, які працюють в тому ж podʼі, що й проксі, мають до них доступ; немає межі довіри між sidecar-проксі та застосунком.
...
Оскільки властивості токена першої сторони є менш захищеними, стандартно Istio використовуватиме токени сторонніх сервісів. Однак ця функція не ввімкнена на всіх платформах Kubernetes.
...
## Налаштування обмежень на кількість downstream-зʼєднань {#configure-a-limit-on-downstream-connections}
