...
{{< gloss "ztunnel" >}}Ztunnel{{< /gloss >}} виступає як проксі на рівні вузла. Він вимагає прав `NET_ADMIN`, `SYS_ADMIN` та `NET_RAW`. Як і [втулок Istio CNI](#istio-cni-plugin), він є **локальними привілеями для вузла**. Ztunnel не має жодних повʼязаних привілеїв Kubernetes RBAC.
...
### Компрометація облікового запису K8s {#k8s-account-compromise}

У цьому випадку скомпрометовано обліковий запис Kubernetes з дозволами на розгортання ресурсів Istio (``networking.istio.io/v1``).

Такий скомпрометований обліковий запис надає зловмиснику контроль для розгортання ресурсів з метою керування трафіком у сервісній мережі. Хоча CRD для керування трафіком є *ресурсами простору імен*, вони впливають на поведінку маршрутизації трафіку від сервісів за межами простору імен, а отже, і на безпеку сервісної мережі.

Наприклад, `VirtualService`, розгорнутий з [опцією mesh gateway](/docs/reference/config/networking/virtual-service/#VirtualService-gateways), уможливлює атаки "людина посередині" в межах сервісної мережі. Сервіс, контрольований зловмисником, може:

* виступати в ролі сервісу призначення.
* перенаправляти трафік на альтернативні призначення.
* відхиляти запити, щоб порушити звʼязок (відмова в обслуговуванні).

Хоча зловмисник може спробувати пересилати трафік до сервісу призначення, щоб читати або змінювати звʼязок між двома сервісами, йому це може не вдатися, оскільки він не може обійти [функції безпеки рівнів 4 та 7](/docs/overview/dataplane-modes/#layer-4-vs-layer-7-features) Istio залежно від конфігурації мережі.

Додаткові деталі та можливі помʼякшення можна знайти в [дописі в блозі *Питання безпеки щодо CRD в Istio з багатокористувацькою архітектурою на основі просторів імен*](/blog/2026/security-considerations-on-namespace-based-multi-tenancy/).
