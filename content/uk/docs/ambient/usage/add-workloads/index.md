...
description: Дізнайтесь як додавати робочі навантаження до ambient mesh.
weight: 10
...
{{< tip >}}
Для того, щоб проксі sidecar використовували варіант сигналізації HBONE/mTLS при взаємодії з цільовими ambient, вони повинні бути налаштовані з `ISTIO_META_ENABLE_HBONE`, встановленим на `true` у метаданих проксі. Це значення стандартно встановлене у `MeshConfig` при використанні профілю `ambient`, тому вам не потрібно робити додаткових налаштувань при використанні цього профілю.
{{< /tip >}}
...
Шлюз ingress може працювати в просторі імен, що не є ambient, і надавати сервіси, що працюють у режимі ambient, sidecar або поза мережею. Взаємодія також підтримується між podʼами в режимі ambient та шлюзами egress Istio. Щоб направити вхідний трафік через цільовий **waypoint** у режимі ambient, використовуйте мітку `istio.io/ingress-use-waypoint`, як описано в розділі [Ingress gateways and waypoints](/docs/ambient/usage/waypoint/#ingress-and-waypoints).
...
| `istio.io/ingress-use-waypoint` | Бета | `Namespace`, `Service` або `ServiceEntry` | Коли `true`, направляє **вхідний** трафік до позначеного міткою ресурсу через шлях цільового waypoint (на додачу до трафіку схід-захід, яким керує `istio.io/use-waypoint`). Вимагає `ENABLE_INGRESS_WAYPOINT_ROUTING` на панелі управління. <br><br> Дійсні значення: `true` або `false`. Див. [Ingress gateways and waypoints](/docs/ambient/usage/waypoint/#ingress-and-waypoints). |
