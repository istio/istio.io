---
title: Виконання WebAssembly модулів
description: Описує, як зробити віддалені WebAssembly модулі доступними в мещі.
weight: 10
aliases:
  - /uk/docs/tasks/extensibility/wasm-module-distribution/
...
## Налаштування модуля Wasm {#configure-a-wasm-module}
...
Щоб налаштувати фільтр WebAssembly з віддаленим модулем Wasm, створіть ресурс `TrafficExtension`:
...
HTTP фільтр буде впроваджено в проксі ingress gateway як фільтр автентифікації. Агент Istio інтерпретує конфігурацію `TrafficExtension`, завантажує віддалені модулі Wasm з реєстру образів OCI до локального файлу і впроваджує HTTP фільтр в Envoy, посилаючись на цей файл.

{{< idea >}}
Якщо ресурс `TrafficExtension` створений у конкретному просторі імен, окрім `istio-system`, будуть налаштовані podʼи в цьому просторі імен. Якщо ресурс створено в просторі імен `istio-system`, будуть впливати всі простори імен.
{{< /idea >}}

## Перевірка модуля Wasm {#verify-the-wasm-module}
...
Дійсні режими: `CLIENT` (вихідний), `SERVER` (вхідний) та `CLIENT_AND_SERVER` (обидва, стандартно).
...
Для отримання додаткових відомостей про API див. [довідку `TrafficExtension`](/docs/reference/config/proxy_extensions/traffic_extension/).
