...
Неправильно, коли два різні ресурси `Telemetry` вибирають одне й те саме робоче навантаження за допомогою `selector`. Аналогічно, не допустимо мати два різні ресурси `Telemetry` у просторі імен без вказаного `selector`.
...
| ---------------- | ------------------------------- |
| `prometheus`     | Метрики                         |
| `stackdriver`    | Трейси (застарілий / обмежений) |
| `envoy`          | Логи доступу                    |

Провайдер `stackdriver` збережено для сумісності із застарілими версіями та обмеженого використання трейсів. Він не є бекендом метрик Telemetry v2 і не експортує логи доступу.
