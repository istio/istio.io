...
{{< tip >}}
Стандартна конфігурація мережі Istio вмикає балансування навантаження за локацією (`localityLbSetting.enabled: true`). Це означає, що налаштування [`OutlierDetection`](/docs/reference/config/networking/destination-rule/#OutlierDetection) у `DestinationRule` **автоматично активує перемикання з урахуванням локації**, навіть якщо ви не встановили `localityLbSetting` у правилі явно. Envoy покладається на виявлення аномалій, щоб визначити несправні точки доступу та ініціювати перемикання на наступну локацію. Щоб відмовитися від цієї неявної поведінки, встановіть `localityLbSetting.enabled: false` у налаштуваннях балансувальника навантаження `DestinationRule`.
{{< /tip >}}
