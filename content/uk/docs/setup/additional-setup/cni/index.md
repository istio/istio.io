...
1. Встановіть Kubernetes з правильно налаштованим втулком primary інтерфейсу CNI. Оскільки [підтримка CNI втулків є обовʼязковою для реалізації мережевої моделі Kubernetes](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/), ймовірно, у вас це вже є, якщо у вас є досить сучасний кластер Kubernetes з функціональною мережею podʼів.
...
Описаний вище механізм відновлення стосується подів, які вже заплановано на вузлі, коли агент CNI ще не готовий. Однак у деяких середовищах (зокрема під час використання засобів автоматичного масштабування) нові вузли можуть стати доступними для планування ще до того, як под `istio-cni` `DaemonSet` буде заплановано на цьому вузлі. Поди, що запускаються на таких вузлах — особливо ті, що мають `restartPolicy: Never`, як, наприклад, поди `Job` у Kubernetes, — можуть зазнати остаточної відмови ще до того, як механізм відновлення встигне втрутитися.

Контролер зняття міток усуває цю першопричину, проактивно контролюючи, коли нові вузли приймають поди з робочим навантаженням. Оператор/власник кластера несе відповідальність за те, щоб постачальник інфраструктури встановив тейнт під час додавання нових вузлів до кластера. Коли контролер untaint увімкнено, він дає вказівку `istiod` автоматично видаляти [тейнт `NoSchedule`](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/) + `cni.istio.io/not-ready`, з вузлів, щойно агент вузла `istio-cni` на цьому вузлі повідомить про готовність, гарантуючи, що жодні под робочих навантажень не будуть заплановані до того, як стане доступним перенаправлення CNI.

{{< tip >}}
Контролер untaint є доповненням до механізму відновлення, а не його заміною. Для повного охоплення слід використовувати обидва: контролер untaint запобігає гонитві на нових вузлах, а механізм відновлення обробляє особливі випадки на вузлах, що вже працюють.
{{< /tip >}}

#### Коли використовувати контролер untaint {#when-to-use-the-untaint-controller}

Увімкніть контролер untaint, якщо ваш кластер використовує автоматичне масштабування вузлів і у вас є навантаження, чутливе до збоїв запуску podʼів через відсутність конфігурації мережі CNI, особливо podи `Job` з `restartPolicy: Never`.

#### Увімкнення контролера untaint {#enabling-the-untaint-controller}

Контролер untaint вимагає двох налаштувань: увімкнення поведінки taint/untaint у чарті CNI та увімкнення контролера в `istiod`:
...
Під час встановлення `istiod` за допомогою Helm передайте такі значення:
...
#### Посилання на конфігурацію {#configuration-reference}

| Параметр                                                 | Опис                                                                                                                                                                                 | Стандартне значення |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| `values.pilot.taint.enabled`                             | Встановлює правила RBAC, необхідні для того, щоб `istiod` міг знімати taint з вузлів.                                                                                                | `false`             |
| `values.pilot.env.PILOT_ENABLE_NODE_UNTAINT_CONTROLLERS` | Увімкнення контролера untaint в `istiod`.                                                                                                                                            | `""` (disabled)     |
| `values.pilot.taint.namespace`                           | Простір імен, у якому працює `istio-cni`; використовується для спостереження за готовністю DaemonSet CNI. За замовчуванням той самий простір імен, що й у `istiod`, якщо не вказано. | `""`                |
...
Контейнери ініціалізації виконуються перед запуском sidecar проксі, що може призвести до втрати трафіку під час їх виконання. Уникайте цієї втрати трафіку, використовуючи одне з наступних налаштувань:
...
Втулок Istio CNI виконує дії для налаштування перенаправлення трафіку podʼа застосунку — в режимі даних sidecar контейнера це означає застосування правил `iptables` у просторі імен мережі podʼа для перенаправлення внутрішнього трафіку podʼа на вбудований sidecar контейнер-проксі Istio.
