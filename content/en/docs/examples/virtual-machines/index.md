...
$ mysql -u root --password < mysqldb-init.sql
{{< /text >}}
...
{{< text bash >}}
$ mysql -u root --password test --execute "select * from ratings;"
+----------+--------+
| ReviewID | Rating |
+----------+--------+
|        1 |      5 |
...
{{< text bash >}}
$ mysql -u root --password test --execute  "update ratings set rating=1 where reviewid=1;select * from ratings;"
+----------+--------+
| ReviewID | Rating |
+----------+--------+
|        1 |      1 |
