...
curl -LO https://raw.githubusercontent.com/istio/istio/release-1.21/samples/bookinfo/src/mysql/mysqldb-init.sql
...
! IFS= read -r -d '' snip_running_mysql_on_the_vm_4_out <<\ENDSNIP
+----------+--------+
| ReviewID | Rating |
+----------+--------+
|        1 |      5 |
...
! IFS= read -r -d '' snip_running_mysql_on_the_vm_5_out <<\ENDSNIP
+----------+--------+
| ReviewID | Rating |
+----------+--------+
|        1 |      1 |
...
! IFS= read -r -d '' snip_reaching_kubernetes_services_from_the_virtual_machine_1_out <<\ENDSNIP
...
