...
in conflict with regards to mTLS. The two resources are in conflict if they each
specify incompatible mTLS modes to be used; this conflict means traffic matching
...
policy requiring mTLS to be used. However, note that without a corresponding
destination rule requiring traffic to use mTLS, traffic will be sent to services
without using mTLS. This conflict means that traffic destined for services in
the mesh will ultimately fail.

In this example, you can fix the issue in one of two ways: you could downgrade
the mesh policy's mTLS requirements to accept plaintext traffic (which might include
removing the mesh policy entirely), or you can create a corresponding
destination rule that specifies to use mTLS for traffic within the mesh:
...
To effectively resolve mTLS conflicts, it's helpful to understand how both
...
2. A policy resource in namespace `my-namespace` that does not contain a
...
1. A destination rule in namespace `other-namespace` that specifies a host that
   ...
   controls the visiblity of configuration resources, will effectively be
   ignored as resources within the same namespace as the source service are
   always visible.
2. A destination rule in namespace `my-namespace` that specifies a host that
   matches `my-service.my-namespace.svc.cluster.local`. This may be an exact
...
3. A destination rule in the "root namespace" (which is, by default,
   ...
   [MeshConfig
   resource](https://istio.io/docs/reference/config/istio.mesh.v1alpha1/#MeshConfig).
...
* Destination rule resource `istio-system/default-rule`, which is requiring mTLS
  when sending traffic to host `myhost.my-namespace.svc.cluster.local:8080`

You can fix the conflict by:

* Modifying policy resource `my-namespace/my-policy` to require mTLS as an
...
* Modifying destination rule `istio-system/default-rule` to not use mTLS by
  ...
  considered the "root namespace" for configuration (although this can be overriden via
  the `rootNamespace` property in the [MeshConfig
  resource](https://istio.io/docs/reference/config/istio.mesh.v1alpha1/#MeshConfig)).
