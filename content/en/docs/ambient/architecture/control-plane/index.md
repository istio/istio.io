...
Ambient uses the Istio {{< gloss >}}control plane{{< /gloss>}}, just like other Istio modes (sidecar, etc). In ambient, rather than the control plane communicating with sidecars in each pod, it communicates with the {{< gloss >}}ztunnel{{< /gloss >}} on each Kubernetes node.
