...
| Value | Behavior |
| --- | --- |
| `PreferClose` | Prioritize endpoints by proximity: network, region, zone, then subzone. Traffic goes to the closest healthy endpoints first. |
| `PreferSameZone` | Same as `PreferClose`. |
...
### PreferClose / PreferSameZone

With `PreferClose`, ztunnel categorizes endpoints by locality and routes to the closest healthy ones:
...
Kubernetes 1.31 introduced the [`spec.trafficDistribution`](https://kubernetes.io/docs/concepts/services-networking/service/#traffic-distribution) field on Services. This Istio annotation provides the same functionality with additional benefits:
