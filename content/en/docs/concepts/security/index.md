...
An authorization policy includes a selector, an action, and a list of rules. For more details, check
...
Policy Scope can be specified with the
`metadata/namespace` field and an optional `selector` field.
If policy target is set to the root namespace (`istio-system` by default), the policy applies to all namespaces in the
mesh. A `selector` field can further restrict policies to apply to specific
workloads. The `selector` uses labels (a list of `{key: value}` pairs) to select the target workload. If not set, the authorization policy applies to all workloads in the
same namespace as the authorization policy.

Value matching in authorization policies supports Exact match, Prefix match with an ending `"*"`,
Suffix match with a starting `"*"`, Presence match with `fieldname: ["*"]` format to specify
that a field must be present.
At the same time, user can use Exclusion matching to match negative conditions like `notValues` in the `when` field, `notIpBlocks`
in the `source` field, `notPorts` in the `to` field. Check [Security Tasks]() for further details.
...
- Custom conditions using the `when` section
- make a workload publicly accessible for both authenticated and unauthenticated identity
