...
[Security Tasks](/docs/tasks/security/) and [Security Best Practice](/docs/ops/best-practices/security/) for detailed
...
In the following sections, we introduce the Istio security features and you can find API usage details  in the [Security Tasks](/docs/tasks/security/) and [Security Configuration](/docs/reference/config/security/) section.

## Istio identity and certificate management
...
determine the identity of a request's origin. Istio securely provisions strong identities
...
instances, such as service names. For more information, check in [Glossary Identity](/docs/reference/glossary/#identity) and [Certificate Management](/docs/tasks/security/cert-management/) tasks.
...
- Peer authentication: used for service-to-service authentication to verify
   ...
   requiring service code changes. If authentication policies use Permissive mode, Istio continues to use plain text between PEPs. To override this behavior explicitly enable mutual TLS mode with [destination rules](/docs/concepts/traffic-management/#destination-rules).
...
- Request authentication: Used for end-user authentication to verify the
   ...
   Connect providers, for example: [ORY Hydra](https://www.ory.sh/), [Keycloak](https://www.keycloak.org/), [Auth0](https://auth0.com/), [Firebase Auth](https://firebase.google.com/docs/auth/), [Google Auth](https://developers.google.com/identity/protocols/OpenIDConnect).
...
along with the keys where appropriate. Upon any policy changes, the new policy is translated to the appropriate configuration telling the PEP how to perform the required authentication mechanisms. Istio sends configurations to the targeted endpoints asynchronously. Once the proxy receives the configuration, the new authentication requirement takes effect immediately on that pod.
...
This mutual TLS solution:

1. Provides each service with a strong identity representing its role to enable interoperability across clusters and clouds.
1. Secures service-to-service communication.
1. Provides a key management system to automate key and certificate generation, distribution, and rotation.

Additionally, Istio supports mutual TLS authentication in [PERMISSIVE mode](/docs/tasks/security/authentication/), which allows a service to accept both
plaintext traffic and mutual TLS traffic at the same time. This feature greatly
improves the mutual TLS onboarding experience. The operator can
gradually install and configure the client's Istio sidecars to send mutual TLS
traffic. Once the configuration of the clients is complete, mutual TLS could be set to [STRICT mode](/docs/tasks/security/authentication/authn-policy/#globally-enabling-istio-mutual-tls-in-strict-mode). For more information, visit the
...
Like other Istio configurations, users could deploy
`.yaml` files using `kubectl`. For more information, check
[Peer Authentication](/docs/reference/config/security/peer_authentication/) and
[Request Authentication](/docs/reference/config/security/request_authentication/) custom resources.
...
help you specify the scope of the policies. Istio applies the narrowest matching policy for each workload using the following order:
...
The following example authentication policy uses [selector](/docs/reference) `app:reviews` specifies that transport authentication for the workloads with the `app:reviews` label must use mutual TLS.

{{< text yaml >}}
apiVersion: security.istio.io/v1beta1
kind: PeerAuthentication
metadata:
  name: "example-peer-authn-policy"
  namespace: "foo"
spec:
  selector:
    matchLabels:
...
with more than one valid JWT are not supported because the output [principal](/docs/reference/config/security/)
of such requests is undefined.

For more information about Istio Authentication, check
[PeerAuthentication](/docs/reference/config/security/peer_authentication/) and
[RequestAuthentication](/docs/reference/config/security/request_authentication/) CRD,
[tasks](/docs/tasks/security/authentication/) and [best practices](/docs/ops/best-practices/security/).
