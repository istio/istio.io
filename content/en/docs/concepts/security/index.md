...
1. Istio Agent generates a private key and CSR, and sends the CSR with its credentials to Citadel for signing.

1. Citadel validates the credentials carried with the CSR, and signs the CSR to generate the certificate.

1. Istio Agent sends both the certificate received from Citadel and the
   private key to Envoy.

1. The above CSR process repeats periodically for certificate and key rotation.

### Istio Agent in Kubernetes

Istio provides the option of using Istio Agent in Kubernetes for certificate and key provisioning, as shown in the figure below.
...
{{< image width="80%" link="./node_agent.svg" caption="PKI with Istio Agent in Kubernetes"  >}}
...
1. Upon receiving the SDS request, Istio Agent creates the private key and CSR before sending the CSR with its credentials to Citadel for signing.

1. Citadel validates the credentials carried in the CSR and signs the CSR to generate the certificate.

1. Istio Agent sends the certificate received from Citadel and the private key to Envoy via the Envoy SDS API.
