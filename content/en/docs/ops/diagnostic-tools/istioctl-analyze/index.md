...
The syntax used for suppression is the same syntax used throughout `istioctl` when referring to resources: '\<kind> \<namespace>.\<name>', or just '\<kind> \<name>' for cluster-scoped resources like `Namespace`. If you want to suppress multiple objects, you can either repeat the `--suppress` argument or use wildcards:
