...
However, the application metrics will follow whatever [Istio configuration](/docs/tasks/security/authentication/authn-policy) has been configured
for the workload.

* If you use `STRICT` mode, TLS must be configured. See below for TLS configuration.
...
* If you use `DISABLE` mode, 
In particular, if [Strict mTLS](/docs/tasks/security/authentication/authn-policy/#globally-enabling-istio-mutual-tls-in-strict-mode)
is enabled, then Prometheus will need to be configured to scrape using Istio certificates.

Note this this only applies to Istio-terminated TLS. If your application directly handles TLS:
