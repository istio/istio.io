...
{{< tip >}}
Available starting in Istio 1.31.
{{< /tip >}}

By default, metrics merging scrapes a single application endpoint per pod, taken from the
`prometheus.io/port` and `prometheus.io/path` annotations. This does not work for pods where
more than one container exposes its own metrics endpoint, for example a primary application
container on `:8080/metrics` alongside a sidecar exporter on `:9100/metrics`. Only the endpoint
named by `prometheus.io/port` is scraped, so metrics from any other container are silently
missing.

The `prometheus.istio.io/scrape-targets` annotation lets a pod declare more than one endpoint.
The value is a comma-separated list of `port:path` pairs; an omitted path defaults to `/metrics`:
...
The pilot-agent scrapes every declared target concurrently and merges the results at
`:15020/stats/prometheus`, in the order the targets are declared. A pod without this annotation
keeps the existing single-target behavior unchanged.

A few limitations to keep in mind:

* Target ports cannot use an Istio-reserved data-plane port (for example, `15020` or `15090`).
  These are rejected at injection time with an error.
* If two targets expose a metric family with the same name, both are included in the merged
  output and Prometheus will fail to parse the response. Keep target metric names disjoint
  across containers.
* Each target's response is capped at 10 MiB; a target that returns more is dropped from the
  merge and counted as a scrape failure.
* If one target fails to respond, the others are still scraped and merged; the failure is
  recorded in `istio_agent_scrape_failures_total{type="application"}`.
