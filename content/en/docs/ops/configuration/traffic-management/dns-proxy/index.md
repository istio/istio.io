...
In addition to capturing application traffic, Istio can also capture DNS requests and to improve the performance and usability of your mesh.
...
Without the DNS capture, a request to `address.internal` would likely fail to resolve. Once this is enabled, we should instead get a response back based on the configured `address`:
...
In the example above, we had a predefined IP address for the service that we wanted to send. However, its common to access external services which do not have stable addresses, and instead rely on DNS. In this case, the DNS proxy will not have enough information to return a response, and will need to forward DNS requests upstream.

This is especially problematic with TCP traffic. Unlike HTTP requests, which are routed based on `Host` headers, TCP carries much less information; we can only route on the destination IP and port number. Because we do not have a stable IP for the backend, we cannot route based on that either, leaving only port number, which leads to conflicts when multiple `ServiceEntry`s for TCP services share the same port.

To work around these issues, the DNS proxy additionally supports automatically allocating addresses for `ServiceEntry`s that do not explicitly define one. This is configured by the `ISTIO_META_DNS_AUTO_ALLOCATE` option mentioned above.
...
We can see the request is sent to an automatically allocated address, `240.240.0.1`. These address will be picked from the `240.240.0.0/16` reserved IP address range to avoid conflicting with real services.
