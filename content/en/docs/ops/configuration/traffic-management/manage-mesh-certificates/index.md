...
To change the type of certificate generated by istiod for itself to use ECC, you must set
`ECC_SIGNATURE_ALGORITHM` to `ECDSA`. There is no need to change this if you wish to use RSA.

It's strength can be configured using either `ECC_CURVE` (ECC based) or `WORKLOAD_RSA_KEY_SIZE` (RSA based).
