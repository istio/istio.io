...
! IFS= read -r -d '' snip_liveness_probe_rewrite_example_3_out <<\ENDSNIP
{
...
! IFS= read -r -d '' snip_liveness_probe_rewrite_example_4_out <<\ENDSNIP
{
...
! IFS= read -r -d '' snip_liveness_and_readiness_probes_using_the_command_approach_4_out <<\ENDSNIP
NAME                             READY     STATUS    RESTARTS   AGE
liveness-6857c8775f-zdv9r        2/2       Running   0           4m
ENDSNIP

! IFS= read -r -d '' snip_disable_the_probe_rewrite_for_a_pod_1 <<\ENDSNIP
kubectl apply -f - <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
...
! IFS= read -r -d '' snip_disable_the_probe_rewrite_for_a_pod_2 <<\ENDSNIP
kubectl apply -f - <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
