...
if [ "$GATEWAY_API" == "true" ]; then
  ...
  updated_snip_download_and_install_10_out=$(echo $snip_download_and_install_10_out | sed -e 's/kubernetes.io/os=linux/.../')
  ...
  updated_snip_download_and_install_8_out=$(echo $snip_download_and_install_8_out | sed -e 's/kubernetes.io/os=linux/.../')
