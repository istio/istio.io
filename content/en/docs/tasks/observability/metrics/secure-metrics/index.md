---
title: Securing Prometheus Scraping for Istio Workloads and Gateways
...
This task demonstrates how to **securely scrape Istio workload and gateway metrics** in sidecar mode using Prometheus over **Istio mTLS**. By default, Prometheus scrapes metrics from Istio workloads and gateways over HTTP. In this task, you configure Istio and Prometheus so that metrics are scraped securely over encrypted connections.

{{< warning >}}
This configuration avoids scraping Istio admin ports (15020/15090) directly over HTTPS and uses a secure fronting port.
{{< /warning >}}
...
* Workload metrics are served from the sidecar admin port (`15020`) or Envoy-only port (`15090`).
* Gateway metrics are served from the gateway pod admin port.
* These endpoints are **not protected by mutual TLS**, and scraping directly over HTTPS is discouraged.

This task replaces the default scraping with a **secure mTLS-enabled configuration**. Prometheus will use a secure fronting port (`9080`) instead of hitting admin ports directly.
...
Ensure Prometheus scrapes metrics securely via the mTLS port (`9080`):
...
$ kubectl annotate pod -n default \
  ...
  prometheus.istio.io/secure-port="9080" \
  --overwrite
{{< /text >}}

These annotations allow Prometheus to discover the httpbin pod and scrape metrics over the secure listener.

1. Create a secure listener on port 9080

Workload metrics can be exposed securely using a sidecar listener on port `9080`. This forwards requests from the secure listener to the sidecar admin port. For Envoy-only metrics, use port `15090`.
...
1. Create Gateway with secure listener on port `9080`.
...
1. ServiceEntry for Gateway admin port (15020 or 15090)
...
{{< text bash >}}
$ kubectl annotate pod -n istio-system <ingress-pod> prometheus.istio.io/secure-port=9080 --overwrite
{{< /text >}}
...
Use `istioctl` to access the Prometheus UI:
...
Verify that the targets for the httpbin workload and the Istio Ingress Gateway are listed with endpoints similar to: `https://<pod-ip>:9080/stats/prometheus   UP`.
Each target should report a status of **UP**.

This confirms that Prometheus is scraping metrics using **HTTPS over Istio mTLS** via the secure fronting port (`9080`), rather than directly accessing the admin ports (`15020` or `15090`).
