...
SLEEP_COUNT=$(count_by_pod default $SLEEP_POD)
HTTPBIN_COUNT=$(count_by_pod default $HTTPBIN_POD)

_verify_same "$SLEEP_COUNT" "0"
_verify_same "$HTTPBIN_COUNT" "0"

# enable access log via Telemetry API
snip_get_started_with_telemetry_api_1
_wait_for_istio telemetry default mesh-logging-default

send_httpbin_requests "status/200"
SLEEP_COUNT_1=$(count_by_pod default $SLEEP_POD)
HTTPBIN_COUNT_1=$(count_by_pod default $HTTPBIN_POD)

verify_geater $SLEEP_COUNT_1 $SLEEP_COUNT
_verify_same $HTTPBIN_COUNT_1 $HTTPBIN_COUNT
...
send_httpbin_requests "status/200"
SLEEP_COUNT_2=$(count_by_pod default $SLEEP_POD)
HTTPBIN_COUNT_2=$(count_by_pod default $HTTPBIN_POD)

_verify_same $SLEEP_COUNT_2 $SLEEP_COUNT_1
verify_geater $HTTPBIN_COUNT_2 $HTTPBIN_COUNT_1
...
send_httpbin_requests "status/200"
SLEEP_COUNT_3=$(count_by_pod default $SLEEP_POD)
HTTPBIN_COUNT_3=$(count_by_pod default $HTTPBIN_POD)

_verify_same $SLEEP_COUNT_3 $SLEEP_COUNT_2
_verify_same $HTTPBIN_COUNT_3 $HTTPBIN_COUNT_2
...
send_httpbin_requests "status/200"
SLEEP_COUNT_4=$(count_by_pod default $SLEEP_POD)
_verify_same $SLEEP_COUNT_4 $SLEEP_COUNT_3

send_httpbin_requests "status/500"
SLEEP_COUNT_5=$(count_by_pod default $SLEEP_POD)
_verify_same $SLEEP_COUNT_5 $SLEEP_COUNT_4
