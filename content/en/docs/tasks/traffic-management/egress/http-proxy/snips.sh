...
! IFS= read -r -d '' snip_deploy_an_https_proxy_8_out <<\ENDSNIP
...
! IFS= read -r -d '' snip_deploy_an_https_proxy_9_out <<\ENDSNIP
...
! IFS= read -r -d '' snip_configure_traffic_to_external_https_proxy_2_out <<\ENDSNIP
<title>Wikipedia, the free encyclopedia</title>
ENDSNIP
...
! IFS= read -r -d '' snip_configure_traffic_to_external_https_proxy_3_out <<\ENDSNIP
...
! IFS= read -r -d '' snip_configure_traffic_to_external_https_proxy_4_out <<\ENDSNIP
1544160065.248    228 172.30.109.89 TCP_TUNNEL/200 87633 CONNECT en.wikipedia.org:443 - HIER_DIRECT/91.198.174.192 -
ENDSNIP
