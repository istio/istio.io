...
! IFS= read -r -d '' snip_route_to_version_1_3_out <<\ENDSNIP
...
! IFS= read -r -d '' snip_route_to_version_1_5_out <<\ENDSNIP
...
spec:
  parentRefs:
...
! IFS= read -r -d '' snip_route_based_on_user_identity_2_out <<\ENDSNIP
apiVersion: networking.istio.io/v1beta1
kind: VirtualService
...
spec:
