---
title: (Experimental) Istio Custom Certificate Authority Integration using Kubernetes CSR API
...
using a custom certificate authority that integrates with the Kubernetes CSR API Interface (<https://kubernetes.io/docs/reference/access-authn-authz/certificate-signing-requests/>). This feature leverages [Chiron](/blog/2019/dns-cert/), a lightweight component linked with Istiod that signs certificates using the Kubernetes CSR API.

This task is split into two parts. The first part demonstrates how to use the Kubernetes CA itself to sign workload certificates. The second part demonstrates how to use an custom CA that integrates with the Kubernetes CSR API to sign your certificates.
...
This task requires the kubeconfig file to point to a running Kubernetes cluster with version >= 1.18

### Deploying Istio with Kubernetes CA

1. Deploy Istio on the cluster using `istioctl` with the feature configuration
...
1. Deploy the `bookinfo` sample application in the bookinfo namespace. Ensure that the instructions below are executed in the Istio directory.
...
1. (Optional) Follow the rest of the steps mentioned in the bookinfo example (<https://istio.io/latest/docs/examples/bookinfo/>) to ensure that communication between services works fine.

### Cleanup Part 1

* To remove the `istio-system` and `bookinfo` namespace:
...
This assumes that the custom CA implements a controller that has the necessary permissions to read and sign Kubernetes CSR Requests. Refer (<https://kubernetes.io/docs/reference/access-authn-authz/certificate-signing-requests/>) for more details. Note that the steps below are dependent on an external-source and may change.

### Deploy Custom CA controller in the Kubernetes cluster

1. For this example, we use an open-source Certificate Authority implemented here
   <https://github.com/cert-manager/signer-ca>. This code builds a controller that reads the CSR resources creates on the Kubernetes cluster and creates certificates using local keys. Follow the instructions on the page to
   1. Build the Certificate-Controller docker image
   1. Upload it to a Docker Registry and
   1. Generate the Kubernetes manifest to deploy the same.

1. Deploy the Kubernetes manifest generated in the step above on your local cluster in the signer-ca-system namespace
...
1. Load the secret into the istiod namespace
    ...
    This step is necessary for Istio to verify that the workload certificates have been signed by the correct certificate authority and adds the root-cert to the trust bundle for mTLS to work.

### Deploying Istio

1. Deploy Istio on the cluster using `istioctl` with the feature configuration applied.
...
When the workloads are deployed above, they send CSR Requests to Istiod which forwards them to the Kubernetes CA for signing. If all goes well, the signed certificates are send back to the workloads where they are installed. To verify that they have indeed been signed by the Kubernetes CA, we need to first extract the signed certificates.
