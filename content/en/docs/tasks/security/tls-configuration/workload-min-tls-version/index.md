...
* Install Istio through `istioctl` with the minimum TLS version configured.
  In the following example, the minimum TLS version for Istio workloads is configured to be 1.3.
...
Within, the `minProtocolVersion` field specifies the minimum TLS version for the TLS connections
among Istio workloads.
