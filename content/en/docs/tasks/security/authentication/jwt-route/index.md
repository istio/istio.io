...
spec:
  ...
  - "*"
  gateways:
  - httpbin-gateway-istio-autogenerated-k8s-gateway-http
  http:
  - match:
    - uri:
        prefix: /headers
...
spec:
  ...
  - matches:
    - path:
        ...
        value: /headers
      headers:
      - name: "ISTIO_JWT_CLAIM"
        value: "request.auth.claims.groups=group1"
    backendRefs:
    - name: httpbin
      port: 8000
EOF
...
1. Validate the ingress gateway routes the request with a valid JWT token that includes the claim `groups: group1`:
    ...
    $ export TOKEN_GROUP=$(curl {{< github_file >}}/security/tools/jwt/samples/groups-scope.jwt -s) && echo "$TOKEN_GROUP" | cut -d '.' -f2 - | base64 --decode
...
1. Validate the ingress gateway returns the HTTP code 404 with a valid JWT but does not include the claim `groups: group1`:
    ...
    {{< text syntax="bash" expandlinks="false" >}}
    $ export TOKEN_NO_GROUP=$(curl {{< github_file >}}/security/tools/jwt/samples/demo.jwt -s) && echo "$TOKEN_NO_GROUP" | cut -d '.' -f2 - | base64 --decode
