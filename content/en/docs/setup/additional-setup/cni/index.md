...
schedulable before the `istio-cni` `DaemonSet` pod has been scheduled on that node. Pods
...
accept workload pods. The cluster operator/owner is responsible for having the taint set by their infrastructure provider when
new nodes are added to the cluster. When enabled, the untaint controller instructs `istiod` to automatically remove the
