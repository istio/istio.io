...
1. Set the name of your cluster as an environment variable. Please be aware
    that cluster names may be autogenerated by your Kubernetes cloud provider.
    Therefore, the cluster name you specify during creation of your cluster may
    be different than the name of the cluster expected here. This will be evident
    by invalid x509 certificate errors in the Istio logs after completing
    installation.
...
1. Execute the following commands on each cluster in the mesh where a virtual
    ...
    $ cp -a samples/certs/"${CLUSTER_NAME}"/workload-cert-chain.pem "${HOME}/"${CLUSTER_NAME}"/cert-chain.pem
...
1. Is this step needed if we always assume there is a load balancer? Try removing.
    The ingress gateway port for Istiod is automatically programmed to 15012 throughout
    the Istio software. Record the ISTIO_PILOT_PORT address in cluster.env.

    {{< text bash >}}
    $ echo "ISTIO_PILOT_PORT=15012" >> ${HOME}/${CLUSTER_NAME}"/cluster.env
    {{< /text >}}
