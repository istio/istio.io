...
kubectl get crd gateways.gateway.networking.k8s.io --context="${CTX_REMOTE_CLUSTER}" &> /dev/null || \
  { kubectl kustomize "github.com/kubernetes-sigs/gateway-api/config/crd?ref=92efbedcc2b40dc097b7ea0eacb894a6033057e1" | kubectl apply -f - --context="${CTX_REMOTE_CLUSTER}"; }
}
