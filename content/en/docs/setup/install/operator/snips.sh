...
istio-1.23.0/bin/istioctl operator init
}
...
istio-system   example-istiocontrolplane1-23-0              HEALTHY   11m
ENDSNIP

snip_canary_upgrade_init() {
istio-1.24.0/bin/istioctl operator init --revision 1-24-0
}

snip_cat_operator_yaml() {
cat example-istiocontrolplane-1-24-0.yaml
}
...
metadata:
  namespace: istio-system
  name: example-istiocontrolplane-1-24-0
spec:
  revision: 1-24-0
  profile: default
ENDSNIP
...
NAME                             READY   STATUS    RESTARTS   AGE
istiod-1-24-0-597475f4f6-bgtcz   1/1     Running   0          64s
...
istiod-1-24-0   ClusterIP   10.111.17.49     <none>        15010/TCP,15012/TCP,443/TCP,15014/TCP           88s
ENDSNIP
