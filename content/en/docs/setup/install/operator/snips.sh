...
! IFS= read -r -d '' snip_kubectl_get_svc_out <<\ENDSNIP
...
! IFS= read -r -d '' snip_kubectl_get_pods_out <<\ENDSNIP
...
! IFS= read -r -d '' snip_verify_operator_cr_out <<\ENDSNIP
...
! IFS= read -r -d '' snip_cat_operator_yaml_out <<\ENDSNIP
apiVersion: install.istio.io/v1alpha1
kind: IstioOperator
metadata:
  namespace: istio-system
...
! IFS= read -r -d '' snip_get_pods_istio_system_out <<\ENDSNIP
NAME                             READY   STATUS    RESTARTS   AGE
...
! IFS= read -r -d '' snip_get_svc_istio_system_out <<\ENDSNIP
NAME            TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)                                         AGE
