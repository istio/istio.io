...
- **Added** support for additional labels exchange for telemetry in sidecar mode.
...
- **Fixed** a issue where `includeInboundPorts: ""` could not work when istio-CNI is enabled.
...
- **Improved** output of `istioctl pc secret` to display trust bundles provided by SPIRE.
...
- **Added** analyzer to report negative status conditions on select Istio and Kubernetes Gateway-API resources.
...
- **Fixed** `istioctl pc secret` performance issue.
