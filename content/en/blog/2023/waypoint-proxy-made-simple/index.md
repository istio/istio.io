...
per-namespace (default) or per-service account. By running outside of the application pod, waypoints
...
| -------------------- | ------------------------- | --------------------------| ------------- |
| Sidecars             | 25 configs * 250 sidecars | 25 configs * 250 sidecars |    12500      |
| Waypoints            | 25 configs * 2 waypoints  | 25 configs * 2 waypoints  |     100       |
| Waypoints ÷ Sidecars |           0.08%           |            0.08%          |     0.08%     |

While we use namespace waypoints to illustrate the simplification above, the simplification is similar
when you apply it to service account waypoints.
...
We are very excited about the waypoint simplification by focusing on destination oriented waypoints. This is another significant step towards simplifying Istio’s usability, scalability and debuggability which are top priorities on Istio’s roadmap. Follow our [getting started guide](http://preliminary.istio.io/latest/docs/ops/ambient/getting-started/) to try the ambient pre-alpha build today and experience the simplified waypoint proxy!