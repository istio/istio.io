...
For requests arriving on port `15008`, which by default is Istio’s inbound HBONE port, the waypoint proxy terminates the HBONE connection and forwards the request to the `main_internal` listener to enforce any workload policies such as AuthorizationPolicy. If you are not familiar with the [internal listener](https://www.envoyproxy.io/docs/envoy/latest/configuration/other_features/internal_listener), it is an Envoy listener that accepts user space connections without using the system network API. The `--waypoint` flag added to the command earlier shows the details of the `main_internal` listener, its filter chains, chain matches, and destinations.
