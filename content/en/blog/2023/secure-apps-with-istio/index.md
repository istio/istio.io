...
In Kubernetes environment, [Istio](https://istio.io/latest/docs/concepts/security/#istio-identity)
...
certificate every 12 hours so that in the event of compromising the compromised certificate only
works for a very limited period of time until the certificate expires therefore could limit the
damage it can cause.
...
[PeerAuthentication](https://istio.io/latest/docs/reference/config/security/peer_authentication/) policy, mesh-wide or
per namespace or workload. In addition, you can also apply Istio’s
[AuthorizationPolicy](https://istio.io/latest/docs/reference/config/security/authorization-policy/) to control access for your workloads.
