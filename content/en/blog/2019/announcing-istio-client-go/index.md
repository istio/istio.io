...
for users to create controllers and perform Create, Read, Update and Delete
...
Istio client go](https://github.com/aspenmesh/istio-client-go) and the [Knative
project](https://github.com/knative/pkg/tree/a805b647f3f2aa6cec2f0490680377e5854775d9/apis/istio).
If you're currently using one of these above mentioned clients, you can easily
...
You can find a more in-depth example [here](https://github.com/istio/client-go/blob/7b65e62d85b04207a1b04ea047dfe635a1e31cb9/cmd/example/client.go).

## Useful tools created for generating Istio client-go

If you're one of the users who's wondering why did it take so long or why was it
difficult to generate this client set, this section is for you. In Istio, we use
Proto specifications to write APIs which are then converted to Go definitions
using the Proto tool chain. There are three major challenges which you might
face if you're trying to generate Kubernetes client set from a Proto generated API:
...
Wrappers](https://github.com/istio/client-go/blob/7b65e62d85b04207a1b04ea047dfe635a1e31cb9/pkg/apis/authentication/v1alpha1/types.gen.go).
...
The annotations which are parsed by this tool and various options
are explained in the [README](https://github.com/istio/tools/blob/master/cmd/kubetype-gen/README.md).
Note that if you're using Proto tools to generate Go types, you would need to
add these annotations as comments in the Proto files, so that the comments are
present in the generated Go files which are used by this tool.
...
canonical way to do this is to create a `deepcopy` function on all nested types.
We created a tool [protoc deep copy
generator](https://github.com/istio/tools/tree/master/cmd/protoc-gen-deepcopy)
which is a `protoc` plugin and can automatically create `deepcopy` functions
...
of the generated `deepcopy` methods.
...
serializing/deserializing to JSON instead of relying on default Golang
...
[example](https://github.com/istio/api/blob/e51134872853a42ff015106ed79d00115e983fba/authentication/v1alpha1/policy_json.gen.go)
