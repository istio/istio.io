...
- **Added** Envoy proxy readiness checks. They now also check Envoy's readiness status.
...
- **Added** metrics to the `/metrics` endpoint of the Citadel Agent on port 15014 to monitor the SDS service.
...
- **Added** `istioctl experimental add-to-mesh` to add Kubernetes services or virtual machines to an existing Istio service mesh.
- **Added** `istioctl experimental remove-from-mesh` to Kubernetes services or virtual machines from an existing Istio service mesh.
