...
1.  Print Mixer access log entries, related to your namespace:
    ...
    Notice `sourceName` and `destinationName` attributes which tell you who called whom, for example _`productpage`_
    called _reviews_. (For _`productpage`_ `sourceName` is unknown since it is called from the outside).
