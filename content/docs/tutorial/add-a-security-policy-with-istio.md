...
1.  Let's define a security policy (a whitelist) that will allow only the _reviews_ microservice to access the _ratings_ microservice:

    {{< text bash >}}
    $ istioctl create -f samples/bookinfo/istio.io-tutorial/whitelist-for-ratings.yaml
    {{< /text >}}

1.  Now let's see that the _sleep_ microservice cannot access the _ratings_ microservice, as expected:
...
1.  Access the webpage of the application and check that it works as expected. It will mean that the _reviews_ microservice can still access the _ratings_ microservice, as expected.
