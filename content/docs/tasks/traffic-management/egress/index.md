...
## Install Istio with access to all external services diabled

Istio has an [installation option](/docs/reference/config/installation-options/) called `global.outboundTrafficPolicy.mode`.  The default value is `ALLOW_ANY`.  When you configure this to `REGISTRY_ONLY`, Istio configures the pod to use iptables to transparently redirect all outbound traffic to the sidecar proxy, which only handles intra-cluster destinations. 

1.  To disable access to all the external services, install or update Istio by using
...
`global.outboundTrafficPolicy.mode` to `REGISTRY_ONLY`: `--set global.outboundTrafficPolicy.mode=REGISTRY_ONLY`.
    ...
    Change `mode` of `outboundTrafficPolicy` from `ALLOW_ANY` to `REGISTRY_ONLY`. Then run `kubectl apply` with the
    edited file.

### Change back to allow any external service policy
...
This option allows access to any external service on any ports without an HTTP service or a service entry within the mesh.
