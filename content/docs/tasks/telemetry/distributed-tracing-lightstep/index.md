...
`productpage.default: proxy client`, which represents the client-side span for the call. It took 15.30 ms. The second
span (labeled with operation `reviews.default.svc.cluster.local:9080/*` and service `reviews.default: proxy server`)
is a child of the first span and represents the server-side span for the call. It took 14.60 ms.
