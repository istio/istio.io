...
1.  Verify that the logs stream has been created and is being populated for
    ...
    $ kubectl -n istio-system logs $(kubectl -n istio-system get pods -l istio-mixer-type=telemetry -o jsonpath='{.items[2].metadata.name}') mixer | grep \"instance\":\"newlog.logentry.istio-system\"
