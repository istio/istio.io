...
1.  Explicitly deny access to version `v3` of the `reviews` service.
    ...
    {{< text bash >}}
    $ istioctl create -f @samples/bookinfo/platform/kube/mixer-rule-deny-label.yaml@
...
1.  Remove the denier configuration that you added in the previous section.
    ...
    {{< text bash >}}
    $ istioctl delete -f @samples/bookinfo/platform/kube/mixer-rule-deny-label.yaml@
    {{< /text >}}
