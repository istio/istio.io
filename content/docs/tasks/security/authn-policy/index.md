...
To configure the client side, you need to set [destination rules](/docs/concepts/traffic-management/destination-rules)
to use mutual TLS. It's possible to use multiple destination rules, one for
each applicable service (or namespace). However, it's more convenient to use a
rule with the `*` wildcard to match all services so that it is on par with the
