...
This task shows how to enable [SDS(secret discovery service)](https://www.envoyproxy.io/docs/envoy/latest/configuration/secret#config-secret-discovery-service) for istio identity provision.
...
Workload requests key/cert from SDS server(node agent, which runs as per-node daemon set) using k8s service account JWT follow
envoy SDS API, node agent generates private key and send CSR request to Citadel, which will sign the certificate chain, and key/cert
will eventually sent back to workload sidecar through SDS server(node agent). Since key/cert which are generated through SDS
are stored in workload sidecar's memory, workload doesn't need to restart to pick up the cert change when rotation happens.
...
## service-to-service mutual TLS using key/cert provisioned through SDS

Follow task in [authentication policy](/docs/tasks/security/authn-policy/) to setup test services.
...
As mentioned, instead of using secret-mount file, key/cert provisioned by SDS flow are stored in workload sidecar's memory,
which is more secure and avoid traffic interruption.
Let's verify no secret-mount file is generated by remote accessing to workload sidecar container that we just deployed.
