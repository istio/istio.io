...
| `request.headers` | map[string, string] | HTTP request headers with lowercased keys. For gRPC, its metadata will be here. | |
...
| `response.headers` | map[string, string] | HTTP response headers with lowercased keys. | |
