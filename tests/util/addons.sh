...
function _wait_for_addon_config_distribution() {
  for addon in "$@"; do
    _wait_for_resourcerce Gateway istio-system "$addon-gateway"
    _wait_for_resourcerce VirtualService istio-system "$addon-vs"
    _wait_for_resourcerce DestinationRule istio-system "$addon"
  done
}
