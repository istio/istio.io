...
Use the `_wait_for_resourcerce` function to allow the change to propogate to the Istio sidecars
before proceeding with the next step of the test:

```sh
snip_config_50_v3 # Step 3: switch 50% traffic to v3
_wait_for_resourcerce virtualservice default reviews # wait for routing change to propagate
```
