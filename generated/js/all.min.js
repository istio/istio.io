const click="click",mouseenter="mouseenter",mouseleave="mouseleave",active="active",keyup="keyup",keydown="keydown",button="button",ariaLabel="aria-label",ariaExpanded="aria-expanded",ariaSelected="aria-selected",ariaControls="aria-controls",tabIndex="tabindex";
...
"use strict";function handleMenu(){queryAll(document,".menu").forEach(a=>{function b(){j.focus()}function c(){l[0].focus()}function d(){l[l.length-1].focus()}function f(a){const b=l.indexOf(a);b<l.length-1?l[b+1].focus():l[0].focus()}function g(a){const b=l.indexOf(a);0<b?l[b-1].focus():l[l.length-1].focus()}function h(a,b){for(let c=a;c<l.length;c++){const a=l[c].textContent.trim().substring(0,1).toLowerCase();if(b===a)return c}return-1}function i(b,c){c=c.toLowerCase();let d=h(l.indexOf(b)+1,c);-1===d&&(d=h(0,c)),-1<d&&(!isActiveOverlay(a)&&(showOverlay(a),toggleAttribute(j,ariaExpanded)),l[d].focus())}const j=query(a,".menu-trigger"),k=query(a,".menu-content");let l=[];for(let b=0;b<k.children.length;b++){const a=k.children[b];"menuitem"===a.getAttribute("role")&&l.push(a)}listen(j,click,b=>{toggleOverlay(a),toggleAttribute(b.currentTarget,ariaExpanded),b.cancelBubble=!0}),listen(j,keydown,b=>{const e=b.key;if(b.ctrlKey||b.altKey||b.metaKey);else if(b.shiftKey)isPrintableCharacter(e)&&i(l[l.length-1],e);else switch(b.keyCode){case keyCodes.SPACE:case keyCodes.RETURN:case keyCodes.DOWN:showOverlay(a),c(),b.preventDefault(),b.cancelBubble=!0;break;case keyCodes.UP:showOverlay(a),d(),b.preventDefault(),b.cancelBubble=!0;break;default:isPrintableCharacter(e)&&i(l[l.length-1],e);}}),l.forEach(a=>{listen(a,keydown,h=>{const e=h.key;if(h.ctrlKey||h.altKey||h.metaKey);else if(h.shiftKey)isPrintableCharacter(e)&&i(a,e);else{switch(h.keyCode){case keyCodes.SPACE:break;case keyCodes.RETURN:const j=new MouseEvent("click",{view:window,bubbles:!0,cancelable:!0,clientX:20});a.dispatchEvent(j);break;case keyCodes.ESC:case keyCodes.TAB:b(),closeActiveOverlay();break;case keyCodes.UP:g(a);break;case keyCodes.DOWN:f(a);break;case keyCodes.HOME:case keyCodes.PAGEUP:c();break;case keyCodes.END:case keyCodes.PAGEDOWN:d();break;default:isPrintableCharacter(e)&&i(a,e);}h.preventDefault(),h.cancelBubble=!0}})})})}handleMenu();
...
"use strict";function handleSidebar(){const a=getById("sidebar");if(null!=a){queryAll(a,".body").forEach(a=>{queryAll(a,button).forEach(a=>{listen(a,click,a=>{let b=a.currentTarget;b.classList.toggle("show");const c=b.nextElementSibling.nextElementSibling;toggleAttribute(c,ariaExpanded);let d=c;do d=d.parentElement;while(!d.classList.contains("body"));d.style.maxHeight=d.scrollHeight+"px"})})}),queryAll(a,".header").forEach(a=>{a.classList.contains("dynamic")&&listen(a,click,()=>{const b=a.nextElementSibling;b.classList.toggle("show"),toggleAttribute(a,ariaExpanded),b.style.maxHeight=b.classList.contains("show")?b.scrollHeight+"px":null})}),queryAll(a,".body").forEach(a=>{if(a.classList.contains("default")){a.style.maxHeight=a.scrollHeight+"px",a.classList.toggle("default"),a.classList.toggle("show");const b=a.previousElementSibling;toggleAttribute(b,ariaExpanded)}});const b=getById("sidebar-toggler");b&&listen(b,click,a=>{getById("sidebar-container").classList.toggle(active),query(a.currentTarget,"svg.icon").classList.toggle("flipped")})}}handleSidebar();
"use strict";function handleTabs(){function a(a,b){queryAll(document,".tabset").forEach(c=>{queryAll(c,".tab-strip").forEach(c=>{c.dataset.cookieName===a&&queryAll(c,"[role=tab]").forEach(a=>{const c=getById(a.getAttribute(ariaControls));a.dataset.cookieValue===b?(a.setAttribute(ariaSelected,"true"),a.setAttribute(tabIndex,"-1"),c.removeAttribute("hidden")):(a.removeAttribute(ariaSelected),a.setAttribute(tabIndex,"-1"),c.setAttribute("hidden",""))})})})}queryAll(document,".tabset").forEach(b=>{function c(a){d(),a.removeAttribute(tabIndex),a.setAttribute(ariaSelected,"true"),getById(a.getAttribute(ariaControls)).removeAttribute("hidden")}function d(){o.forEach(a=>{a.setAttribute(tabIndex,"-1"),a.setAttribute(ariaSelected,"false")}),n.forEach(a=>{a.setAttribute("hidden","")})}function f(){o[0].focus()}function g(){o[o.length-1].focus()}function h(a){const b=o.indexOf(a);b<o.length-1?o[b+1].focus():o[0].focus()}function i(a){const b=o.indexOf(a);0<b?o[b-1].focus():o[o.length-1].focus()}function j(a,b){for(let c=a;c<o.length;c++){const a=o[c].textContent.trim().substring(0,1).toLowerCase();if(b===a)return c}return-1}function k(a,b){b=b.toLowerCase();let c=j(o.indexOf(a)+1,b);-1===c&&(c=j(0,b)),-1<c&&o[c].focus()}const l=query(b,".tab-strip");if(null===l)return;const m=l.dataset.cookieName,n=queryAll(b,"[role=tabpanel]"),o=[];if(queryAll(l,"[role=tab]").forEach(a=>{o.push(a)}),m){const b=readCookie(m);b&&a(m,b)}queryAll(l,button).forEach(b=>{listen(b,"focus",()=>{c(b),m&&(createCookie(m,b.dataset.cookieValue),a(m,b.dataset.cookieValue))}),listen(b,"click",()=>{c(b),m&&(createCookie(m,b.dataset.cookieValue),a(m,b.dataset.cookieValue))}),listen(b,keydown,a=>{const c=a.key;if(a.ctrlKey||a.altKey||a.metaKey);else if(a.shiftKey)isPrintableCharacter(c)&&k(b,c);else{switch(a.keyCode){case keyCodes.LEFT:i(b);break;case keyCodes.RIGHT:h(b);break;case keyCodes.HOME:f();break;case keyCodes.END:g();break;default:isPrintableCharacter(c)&&k(b,c);}a.preventDefault(),a.cancelBubble=!0}})})})}handleTabs();
