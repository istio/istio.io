...
"use strict";function handleMenu(){queryAll(document,".menu").forEach(a=>{function b(){c.focus()}const c=query(a,".menu-trigger"),d=query(a,".menu-content");let e=[];for(let b=0;b<d.children.length;b++){const a=d.children[b];"menuitem"===a.getAttribute("role")&&e.push(a)}const f=new KbdNav(e);listen(c,click,b=>{toggleOverlay(a),toggleAttribute(b.currentTarget,ariaExpanded),b.cancelBubble=!0}),listen(c,keydown,b=>{const c=b.key;switch(b.keyCode){case keyCodes.SPACE:case keyCodes.RETURN:case keyCodes.DOWN:showOverlay(a),f.focusFirstElement();break;case keyCodes.UP:showOverlay(a),f.focusLastElement();break;default:return void(isPrintableCharacter(c)&&f.focusElementByChar(c));}b.stopPropagation(),b.preventDefault()}),e.forEach(a=>{listen(a,keydown,c=>{const d=c.key;if(c.ctrlKey||c.altKey||c.metaKey);else if(c.shiftKey)isPrintableCharacter(d)&&f.focusElementByChar(d);else{switch(c.keyCode){case keyCodes.SPACE:break;case keyCodes.RETURN:const e=new MouseEvent("click",{view:window,bubbles:!0,cancelable:!0,clientX:20});a.dispatchEvent(e);break;case keyCodes.ESC:case keyCodes.TAB:return b(),void closeActiveOverlay();case keyCodes.UP:f.focusPrevElement();break;case keyCodes.DOWN:f.focusNextElement();break;case keyCodes.HOME:case keyCodes.PAGEUP:f.focusFirstElement();break;case keyCodes.END:case keyCodes.PAGEDOWN:f.focusLastElement();break;default:return void(isPrintableCharacter(d)&&f.focusElementByChar(d));}c.stopPropagation(),c.preventDefault()}})})})}handleMenu();
