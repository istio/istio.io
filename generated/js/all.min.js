...
"use strict";function handleHeader(){function a(){getById(c).classList.remove(g),getById(d).classList.remove(g),getById(f).value=""}function b(){getById(c).classList.add(g),getById(d).classList.add(g),getById(f).focus()}const c="search-form",d="header-links",f="search-textbox",g="show-search",h="open-hamburger";listen(document.body,"keyup",b=>{27===b.which&&a()}),listen(getById("search-show"),click,a=>{a.preventDefault(),b()}),listen(getById("search-close"),click,b=>{b.preventDefault(),a()}),listen(getById("search-form"),"submit",b=>{b.preventDefault();const c=getById(f),d=getById("search-page-url"),e=d.value+"?q="+c.value;a(),window.location.assign(e)}),listen(getById("hamburger"),click,()=>{getById("brand").classList.toggle(h),getById(d).classList.toggle(h),getById(c).classList.toggle(h),getById(f).focus()})}handleHeader();
"use strict";function handleSidebar(){const a=getById("sidebar");if(null!=a){queryAll(a,"button").forEach(a=>{listen(a,click,a=>{let b=a.currentTarget;b.classList.toggle("show");const c=b.nextElementSibling.nextElementSibling;"true"===c.getAttribute("aria-expanded")?c.setAttribute("aria-expanded","false"):c.setAttribute("aria-expanded","true");let d=c;do d=d.parentElement;while(!d.classList.contains("body"));d.style.maxHeight=d.scrollHeight+"px"})}),queryAll(a,".header").forEach(a=>{listen(a,click,()=>{const b=a.nextElementSibling;b.classList.toggle("show"),b.style.maxHeight=b.classList.contains("show")?b.scrollHeight+"px":null})}),queryAll(a,".body").forEach(a=>{a.classList.contains("default")&&(a.style.maxHeight=a.scrollHeight+"px",a.classList.toggle("default"),a.classList.toggle("show"))});const b=getById("sidebar-toggler");b&&listen(b,click,a=>{getById("sidebar-container").classList.toggle(active),query(a.currentTarget,"svg.icon").classList.toggle("flipped")})}}handleSidebar();
"use strict";function handleTabs(){queryAll(document,"[role=tablist]").forEach(a=>{queryAll(a,".tab-strip").forEach(a=>{const b=a.dataset.cookieName;if(b){const c=readCookie(b);c&&queryAll(a,"a").forEach(a=>{a.dataset.cookieValue===c?(a.classList.add(active),getById(a.dataset.tab).classList.add(active)):(a.classList.remove(active),getById(a.dataset.tab).classList.remove(active))})}queryAll(a,"button").forEach(c=>{listen(c,click,()=>{queryAll(a,"button").forEach(a=>{a.classList.remove(active),getById(a.dataset.tab).classList.remove(active)}),c.classList.add(active),getById(c.dataset.tab).classList.add(active),null!==b&&createCookie(b,c.dataset.cookieValue)})})})})}handleTabs();
